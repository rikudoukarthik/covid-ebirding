...
# to plot positive and negative separately
...
levelplot(rast_SoIBmap_a, 
          ...
          main = "Negative log prop. change in monthly (six months) no. of lists per 24kmx24km cell") +
...
levelplot(rast_SoIBmap_b, 
          ...
          margin = F, 
          at = seq(plotfrom, plotto, 0.1),
          main = "Positive log prop. change in monthly (six months) no. of lists per 24kmx24km cell") +
...
temp1 <- temp0 %>% 
  mutate(BeforeDuring = raster::extract(rast_SoIBmap$BeforeDuring, 
                                        cbind(CELL.LONG, CELL.LAT)),
         DuringAfter = raster::extract(rast_SoIBmap$DuringAfter, 
                                        cbind(CELL.LONG, CELL.LAT))) %>% 
  pivot_longer(cols = c(BeforeDuring, DuringAfter),
               names_to = "PERIOD", 
               values_to = "LOG.PROP")  # log. prop. change in monthly no.lists per 24x24
  
data_SoIBmap <- temp1 %>% 
  group_by(PERIOD) %>% 
  summarise(LOG.PROP.SE = boot_se(LOG.PROP),
            LOG.PROP.SD = sd(LOG.PROP),
            LOG.PROP = mean(LOG.PROP),
            LOG.PROP.CI.L = LOG.PROP - 1.96*LOG.PROP.SE,
            LOG.PROP.CI.U = LOG.PROP + 1.96*LOG.PROP.SE)
 
changepalette <- c(covid_palette[2], covid_palette[4])
changepalette2 <- c(covid_palette[2], covid_palette[4])


((ggplot(temp1, aes(x = PERIOD, y = LOG.PROP, colour = PERIOD)) + 
    geom_bin_2d(colour = NA, size = 0.005) +
    scale_fill_viridis_c() +
    scale_colour_manual(values = changepalette) +
    labs(title = "Log prop. change in monthly no. of lists per 24kmx24km grid cell",
         subtitle = "All 24kmx24km cells with birding activity",
         x = "Period", y = "Log proportional change")) |
(ggplot(data_SoIBmap, aes(x = PERIOD, y = LOG.PROP, colour = PERIOD)) + 
    geom_point(data = temp1, alpha = 0.1, position = position_jitter()) +
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = LOG.PROP.CI.L, ymax = LOG.PROP.CI.U), 
                  size = 1.25, width = 0.1, position = position_dodge(0.5)) +
    scale_colour_manual(values = changepalette) +
    scale_y_continuous(limits = c(-0.1, 0.12)) +
    labs(title = "Net log prop. change in monthly no. of lists at national level",
         subtitle = "Averaged across all 24kmx24km cells with birding activity",
         x = "Period", y = "Log proportional change"))) +
plot_layout(guides = "collect") -> s_spread_netchange
