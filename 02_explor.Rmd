...
ggsave("02_explor_figs/10a_s_UNU_lists.png", 
       plot = s_UNU_lists,
       width = 9, height = 9, units = "in")
...
ggsave("02_explor_figs/10b_s_UNU_cells.png",
       plot = s_UNU_cells,
       width = 9, height = 9, units = "in")
...
##### Proportion of total grid cells & total urban vs. non-urban cells covered ####
...
ggsave("02_explor_figs/11a_s_cover.png", 
       plot = s_cover,
       width = 9, height = 9, units = "in")
...
##### Proportion of total urban cells covered ####
...
temp2 <- temp1 %>% 
  ...
  dplyr::summarise(PROP.SUBCELLS = mean(PROP.SUBCELLS),
                   SE = sqrt(sum((SE)^2))/n(),
                   CI.L = PROP.SUBCELLS - 1.96*SE,
                   CI.U = PROP.SUBCELLS + 1.96*SE)
...
ggsave("02_explor_figs/11b_s_cover_U.png", 
       plot = s_cover_U,
       width = 9, height = 9, units = "in")

ggsave("02_explor_figs/11b_s_cover_NU.png", 
       plot = s_cover_NU,
       width = 9, height = 9, units = "in")
...
##### Change in birding effort (no. of lists) in each 2x2 UNU cell averaged across 25x25 cell  ####
...
data_UNUmap0 <- data_UNU %>% 
  ...
  group_by(CELL.ID, URBAN, SUBCELL.ID, SUBCELL.LONG, SUBCELL.LAT, COVID, MONTH) %>% 
  dplyr::summarise(NO.LISTS = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>% 
  # summarise removes one grouping variable so no need to repeat group_by() 
  dplyr::summarise(NO.LISTS = mean(NO.LISTS)) %>% 
  ungroup()
...
temp0 <- data_UNUmap0 %>% 
  distinct(CELL.ID, URBAN, SUBCELL.ID, SUBCELL.LONG, SUBCELL.LAT)


data_UNUmap1 <- data_UNUmap0 %>% 
  filter(COVID == "BEF") %>% 
  # to include subcells not visited in BEF
  right_join(temp0) %>% 
  mutate(COVID = "BEF", 
         NO.LISTS = replace_na(NO.LISTS, replace = 0)) %>% 
  dplyr::select(SUBCELL.LONG, SUBCELL.LAT, NO.LISTS)
...
temp1a <- temp1 %>% 
  filter(URBAN == 1) %>% 
  dplyr::select(CELL.LONG, CELL.LAT, CHANGE)
...
levelplot(rast_UNUmapU_a, 
          ...
          main = "Negative log prop. change in monthly (seven months) urban lists per 24kmx24km cell") +
...
levelplot(rast_UNUmapU_b, 
          ...
          margin = F, 
          at = seq(plotfromU, plottoU, 0.03),
          main = "Positive log prop. change in monthly (seven months) urban lists per 24kmx24km cell") +
  latticeExtra::layer(sp::sp.polygons(in_bound, alpha = 0.4)) -> s_spread_UNUmapU_b

levelplot(rast_UNUmapU, 
          par.settings = mytheme, 
          margin = F, 
          at = seq(plotfromU, plottoU, 0.03),
          main = "Log prop. change in monthly (seven months) urban lists per 24kmx24km cell") +
  latticeExtra::layer(sp::sp.polygons(in_bound, alpha = 0.4)) -> s_spread_UNUmapU
...
levelplot(rast_UNUmapN_a, 
          ...
          main = "Negative log prop. change in monthly (seven months) non-urban lists per 24kmx24km cell") +
...
levelplot(rast_UNUmapN_b, 
          ...
          margin = F, 
          at = seq(plotfromN, plottoN, 0.03),
          main = "Positive log prop. change in monthly (seven months) non-urban lists per 24kmx24km cell") +
  latticeExtra::layer(sp::sp.polygons(in_bound, alpha = 0.4)) -> s_spread_UNUmapN_b

levelplot(rast_UNUmapN, 
          par.settings = mytheme, 
          margin = F, 
          at = seq(plotfromN, plottoN, 0.03),
          main = "Log prop. change in monthly (seven months) non-urban lists per 24kmx24km cell") +
  latticeExtra::layer(sp::sp.polygons(in_bound, alpha = 0.4)) -> s_spread_UNUmapN
...
png(file = "02_explor_figs/12b_s_spread_UNUmapU.png", 
    width = 17, height = 8, units = "in", res = 300)
print(s_spread_UNUmapU)
dev.off()
...
png(file = "02_explor_figs/12b_s_spread_UNUmapN.png", 
    width = 17, height = 8, units = "in", res = 300)
print(s_spread_UNUmapN)
dev.off()



##### Change in monthly UNU ratio in each 24x24 cell  ####
...
data_UNUpropmap1 <- data_UNUpropmap0 %>% 
  ...
  dplyr::select(CELL.LONG, CELL.LAT, PROP.UNU) 
...
rast_UNUpropmap <- raster::stack(
  raster::overlay(x = rast_UNUpropmap1, y = rast_UNUpropmap2, 
                  k = 0.00001, # not 1, since this is prop.
                  fun = rast_logpropchange),
...
levelplot(rast_UNUpropmap_a, 
          ...
          main = "Negative log prop. change in monthly (seven months) urban birding proportion per 24kmx24km cell") +
...
levelplot(rast_UNUpropmap_b, 
          ...
          margin = F, 
          at = seq(plotfrom, plotto, 0.01),
          main = "Positive log prop. change in monthly (seven months) urban birding proportion per 24kmx24km cell") +
...
ggsave("02_explor_figs/13_t_spread_dow.png", 
       plot = t_spread_dow,
       width = 9, height = 9, units = "in")
...
ggsave("02_explor_figs/14_t_spread_tod.png", 
       plot = t_spread_tod,
