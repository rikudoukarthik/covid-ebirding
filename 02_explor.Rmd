...
data_SoIBmap <- temp1 %>% 
  group_by(PERIOD) %>% 
  summarise(PROP.SE = boot_se(PROP.CHANGE),
            PROP.CHANGE.SD = sd(PROP.CHANGE),
            PROP.CHANGE = mean(PROP.CHANGE),
            PROP.CHANGE.CI.L = PROP.CHANGE - 1.96*PROP.SE,
            PROP.CHANGE.CI.U = PROP.CHANGE + 1.96*PROP.SE)
...
ggplot(data_SoIBmap, aes(x = PERIOD, y = PROP.CHANGE, colour = PERIOD)) + 
  geom_point(size = 3, position = position_dodge(0.5)) +
  geom_errorbar(aes(ymin = PROP.CHANGE.CI.L, ymax = PROP.CHANGE.CI.U), 
...
ggsave("02_explor_figs/12d_s_spread_netchange.png", 
       ...
       dpi = 300, width = 7, height = 10, units = "in")
