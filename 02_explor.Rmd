...
data_spatlists <- data_UNU %>% 
  ungroup() %>% 
  select(STATE, COUNTY, LATITUDE, LONGITUDE, OBSERVATION.DATE, TIME.OBSERVATIONS.STARTED,
         SAMPLING.EVENT.IDENTIFIER, GROUP.IDENTIFIER, GROUP.ID, YEAR, MONTH, DAY.M, COVID,
         NO.SP, TIME.D, URBAN, NONURBAN, SUBCELL.ID, CELL.ID) %>% 
  group_by(SAMPLING.EVENT.IDENTIFIER) %>% 
  slice(1) 

save(data_spatlists, file = "data/data_spatlists.RData")
...
# load("data/data_spatlists.RData")
...
rast_SoIBmap_a <- rast_SoIBmap
rast_SoIBmap_a[rast_SoIBmap_a >= 0] <- NA

rast_SoIBmap_b <- rast_SoIBmap
rast_SoIBmap_b[rast_SoIBmap_b <= 0] <- NA


mytheme <- rasterTheme(region = viridis::viridis(100),
                       pch = 19, cex = 0.7,
...
levelplot(rast_SoIBmap_a, 
          par.settings = mytheme, 
          margin = F, 
          at = seq(plotfrom, plotto, 0.1),
          main = "Negative log prop. change in monthly (six months) no. of lists per 25kmx25km cell") +
  latticeExtra::layer(sp::sp.polygons(in_bound, alpha = 0.4)) -> s_spread_SoIBmap_a

levelplot(rast_SoIBmap_b, 
          par.settings = mytheme, 
          margin = F, 
          at = seq(plotfrom, plotto, 0.1),
          main = "Positive log prop. change in monthly (six months) no. of lists per 25kmx25km cell") +
  latticeExtra::layer(sp::sp.polygons(in_bound, alpha = 0.4)) -> s_spread_SoIBmap_b



png(file = "02_explor_figs/12a_s_spread_SoIBmap_a.png", 
    width = 17, height = 8, units = "in", res = 300)
print(s_spread_SoIBmap_a)
dev.off()

png(file = "02_explor_figs/12a_s_spread_SoIBmap_b.png", 
    width = 17, height = 8, units = "in", res = 300)
print(s_spread_SoIBmap_b)
dev.off()
...
levelplot(rast_UNUmapU_a, 
          ...
          main = "Negative log prop. change in monthly (six months) urban lists per 25kmx25km cell") +
...
levelplot(rast_UNUmapU_b, 
          ...
          margin = F, 
          at = seq(plotfromU, plottoU, 0.03),
          main = "Positive log prop. change in monthly (six months) urban lists per 25kmx25km cell") +
...
levelplot(rast_UNUmapN_a, 
          ...
          main = "Negative log prop. change in monthly (six months) non-urban lists per 25kmx25km cell") +
...
levelplot(rast_UNUmapN_b, 
          ...
          margin = F, 
          at = seq(plotfromN, plottoN, 0.03),
          main = "Positive log prop. change in monthly (six months) non-urban lists per 25kmx25km cell") +
...
temp0 <- data_UNUpropmap0 %>% 
  group_by(COVID, CELL.ID, MONTH) %>% 
  filter(URBAN == 1) %>% 
  dplyr::summarise(U.LISTS = NO.LISTS) %>% 
  ungroup()

temp1 <- data_UNUpropmap0 %>% 
  group_by(COVID, CELL.ID, MONTH) %>% 
...
data_UNUpropmap0 <- data_UNUpropmap0 %>% 
  left_join(temp0) %>% 
  left_join(temp1) %>% 
  mutate(U.LISTS = ifelse(is.na(U.LISTS), 0, U.LISTS)) %>% 
  group_by(COVID, CELL.ID, MONTH) %>% 
...
data_UNUpropmap1 <- data_UNUpropmap0 %>% 
  mutate(PROP.UNU = case_when(COVID == "BEF" ~ PROP.UNU,
                              TRUE ~ 0 + k)) %>% 
...
data_UNUpropmap2 <- data_UNUpropmap0 %>% 
  mutate(PROP.UNU = case_when(COVID %in% c("DUR_20","DUR_21") ~ PROP.UNU,
                              TRUE ~ 0 + k)) %>% 
  group_by(CELL.ID, CELL.LONG, CELL.LAT) %>% 
  dplyr::summarise(PROP.UNU = case_when(n() > 1 ~ mean(PROP.UNU), 
                                 TRUE ~ PROP.UNU),
            COVID = "DUR") %>% 
  ungroup() %>% 
  select(CELL.LONG, CELL.LAT, PROP.UNU) 
...
data_UNUpropmap3 <- data_UNUpropmap0 %>% 
  mutate(PROP.UNU = case_when(COVID == "AFT" ~ PROP.UNU,
                              TRUE ~ 0 + k)) %>% 
  select(CELL.LONG, CELL.LAT, PROP.UNU)
...
levelplot(rast_UNUpropmap_a, 
          ...
          main = "Negative log prop. change in monthly (six months) urban birding proportion per 25kmx25km cell") +
...
levelplot(rast_UNUpropmap_b, 
          ...
          margin = F, 
          at = seq(plotfrom, plotto, 0.01),
          main = "Positive log prop. change in monthly (six months) urban birding proportion per 25kmx25km cell") +
...
rm(list = setdiff(ls(), 
                  c("covid_palette", "covid_palette3",
                    # "data_UNU", "rast_UNU", 
                    "boot_se", "dataqual_filt", "rast_agg_fn", "rast_logpropchange")))
