...
# filtering the data for current use (8)
...
dist_sw <- data0 %>% 
  ...
  group_by(COVID, YEAR, MONTH, STATE) %>% 
  dplyr::summarise(DIST = mean(EFFORT.DISTANCE.KM)) %>% 
  group_by(COVID, YEAR, MONTH, STATE) %>% 
  dplyr::summarise(DIST = boot_conf(x = DIST)) %>% 
  group_by(COVID, YEAR, MONTH, STATE) %>% 
...
dur_sw <- data0 %>% 
  ...
  ungroup() %>% 
  group_by(COVID, YEAR, MONTH, STATE) %>% 
  dplyr::summarise(DURATION = mean(DURATION.MINUTES)) %>% 
  group_by(COVID, YEAR, MONTH, STATE) %>% 
  dplyr::summarise(DURATION = boot_conf(x = DURATION)) %>% 
  group_by(COVID, YEAR, MONTH, STATE) %>% 
...
length_sw <- !!!!!!!!! %>% 
  ...
  summarise(LENGTH = boot_conf(x = LENGTH)) %>% 
  group_by(COVID, YEAR, MONTH, STATE) %>% 
