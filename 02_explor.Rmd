...
rm(list = setdiff(ls(), 
                  c("covid_palette", "covid_palette3", "india", "states",
                    "data0_sliceG", "data0_sliceS", "month_compar",
                    # "data_UNU", "rast_UNU", 
...
t_dow_sw <- data0_slice_S %>% 
  ...
  group_by(COVID, STATE, DAY.W, MONTH) %>% 
...
(ggplot(t_dow_nw, 
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "Temporal spread (DoW) of checklists at national level (Sep to Mar)",
...
ggplot(filter(t_dow_sw, STATE == "Maharashtra"), 
    ...
    labs(title = "Maharashtra",
         x = "Day", y = "Proportion of lists")) +
plot_layout(guides = "collect") -> t_spread_dow
...
t_tod_sw <- data0_slice_S %>% 
  ...
  group_by(COVID, MONTH, STATE) %>% 
  mutate(NO.LISTS = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>%
  group_by(COVID, STATE, HOUR, MONTH) %>% 
  dplyr::summarise(NO.LISTS = min(NO.LISTS),
...
(ggplot(t_tod_nw, 
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "Temporal spread (ToD) of checklists at national level (Sep to Mar)",
...
ggplot(filter(t_tod_sw, STATE == "Maharashtra"), 
    ...
    labs(title = "Maharashtra",
         x = "Time (hours)", y = "Proportion of lists")) +
plot_layout(guides = "collect") -> t_spread_tod
