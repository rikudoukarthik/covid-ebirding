...
gg_b_model <- function(data, type, data_points) {
  ...
  if (type == "overall_ribbon") {
    ...
    data <- data |> 
      mutate(MONTHS.TYPE = factor(MONTHS.TYPE, levels = c("LD", "ALL"))) |> 
      # remove 2018 points cos baseline
      filter(M.YEAR != 2018) |> 
    ...
    plot_list <- map(levels(data$MONTHS.TYPE), ~ {
      ...
      data_points_sub <- data_points |> filter(MONTHS.TYPE == .x)
      ...
      ggplot(data = data_sub, 
        ...
        geom_point(data = data_points_sub, 
                   mapping = aes(group = PATH.GROUP),
                   size = 3, alpha = 0.5, position = position_dodge(0.5)) + 
        geom_path(data = data_points_sub, 
                  mapping = aes(x = as.numeric(M.YEAR), y = PRED.PERC, group = PATH.GROUP),
                  linewidth = 1, alpha = 0.35, position = position_dodge(0.5)) + 
