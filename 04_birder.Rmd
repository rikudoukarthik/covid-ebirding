...
data_a <- data0_MY_slice_S %>% 
  ...
  dplyr::summarise(G.LISTS = n_distinct(SAMPLING.EVENT.IDENTIFIER),
                   TOT.LISTS = min(TOT.LISTS),
                   PROP.GROUP = G.LISTS/TOT.LISTS) %>% 
  ungroup()

tictoc::tic(glue::glue("GLMM for India"))
model_a <- glmer(PROP.GROUP ~ M.YEAR + M.YEAR:MONTH + (1|OBSERVER.ID),
                 data = data_a, weights = TOT.LISTS,
...
count <- 0
for (j in 1:n_distinct(data0_a$MONTH)) {
  for (k in 1:n_distinct(data0_a$M.YEAR)) {
    
    count <- count + 1
    
    tictoc::tic(glue::glue("Bootstrapped predictions for India in month {j} of MY {data0_a$M.YEAR[count]}"))
    prediction <- boot_conf_GLMM(model_a,
                                 new_data = data.frame(MONTH = data0_a$MONTH[count],
                                                       M.YEAR = data0_a$M.YEAR[count]),
                                 nsim = 100)
    tictoc::toc()
    
    data0_a$PRED.LINK[count] <- median(na.omit(prediction$t[count]))
    data0_a$SE.LINK[count] <- sd(na.omit(prediction$t[count]))
...
for (i in 1:4) { # for each state
  ...
  for (j in 1:n_distinct(data0_b$MONTH)) {
    for (k in 1:n_distinct(data0_b$M.YEAR)) {
      
      count <- count + 1
      
      tictoc::tic(glue::glue("Bootstrapped predictions for {anal_states[, i]} in month {j} of MY {data0_a$M.YEAR[count]}"))
      prediction <- boot_conf_GLMM(model_b,
                                   new_data = data.frame(MONTH = data0_b$MONTH[count],
                                                         M.YEAR = data0_b$M.YEAR[count]),
                                   nsim = 100)
      tictoc::toc()
      
      data0_b$PRED.LINK[count] <- median(na.omit(prediction$t[count]))
      data0_b$SE.LINK[count] <- sd(na.omit(prediction$t[count]))
