...
grid_dist_links <- grid_dist %>% 
  ...
  mutate(DISTRICT.CELLS.TOT = n_distinct(CELL.ID)) %>% 
  ungroup()
...
temp0 <- data0_MY_slice_G %>% 
  ...
  # established district links will be considered (removes 3 cells)
  dplyr::select(-DISTRICT) %>% # removing to avoid confusion
  inner_join(grid_dist)

# summarising cell metrics
temp1 <- temp0 %>% 
  group_by(COVID, CELL.ID) %>% 
  dplyr::summarise(CELL.LISTS = n_distinct(GROUP.ID)) %>% 
  ungroup() %>% 
  # filling zeroes for all grid cells under consideration (in each time period)
  complete(COVID, CELL.ID, fill = list(CELL.LISTS = 0)) %>% 
  left_join(grid_dist) %>% 
  st_drop_geometry()
...
data_spread0 <- temp0 %>% 
  ...
  group_by(COVID, DISTRICT.NAME, DISTRICT.CELLS.TOT) %>% 
  summarise(DISTRICT.LISTS = n_distinct(GROUP.ID),
            DISTRICT.CELLS = n_distinct(CELL.ID)) %>% 
  ungroup() %>% 
  complete(COVID, nesting(DISTRICT.NAME, DISTRICT.CELLS.TOT),
           fill = list(DISTRICT.LISTS = 0, DISTRICT.CELLS = 0)) %>% 
  mutate(DISTRICT.CELLS.COV = 100*DISTRICT.CELLS/DISTRICT.CELLS.TOT) %>% 
  right_join(temp1) %>% 
  # moving COVID to columns
  dplyr::select(-DISTRICT.CELLS) %>% 
  pivot_longer(cols = c("CELL.LISTS", "DISTRICT.LISTS", "DISTRICT.CELLS.COV"),
               names_to = "METRIC", values_to = "VALUE") %>% 
  pivot_wider(names_from = "COVID", values_from = "VALUE") %>% 
  # calculating change (produces NA for x == 0 & y == 0 which need to be removed)
  mutate(CHANGE1 = s_spread_change(BEF, DUR),
         CHANGE2 = s_spread_change(DUR, AFT)) %>% 
  # joining grid cell geometry
  left_join(gridmapg1_IN, by = "CELL.ID", suffix = c(".district", ".cell"))

transition_names <- data.frame(T.CODE = c("T1", "T2"),
                               T.LABEL = c("Before to During", "During to After"))


##### Proportional change in birding effort per grid cell ####
...
data_spread <- data_spread0 %>% 
  filter(METRIC == "CELL.LISTS")

s_spread_map <- ggplot(data_spread) +
  geom_polygon(data = indiamap,
               aes(long, lat, group = group),
               colour = "black", fill = "#E6E9F1", size = 0.2) +
  geom_sf(aes(fill = CHANGE1, geometry = geometry.cell), col = NA, na.rm = T) +
  scale_fill_viridis_c(option = "cividis",
                       name = "Proportional\nchange") +
  labs(x = "Longitude", y = "Latitude",
       title = "Proportional change in no. of lists per 25km \u00d7 25km cell in transition periods") +
...
##### Proportional change in birding effort per district ####
...
ggplot(data_spread) +
  ...
  scale_fill_viridis_c(option = "cividis",
                       name = "Proportional\nchange") +
  labs(x = "Longitude", y = "Latitude",
       title = "Proportional change in no. of lists per district in transition periods") +
  # making the colourbar bigger
  theme(legend.key.height = unit(1, "inches")) # height between two breaks of legend
