...
# 3. distribution of proportional change
# 4. with SEs of proportional change
...
data_spread3 <- data_spread0 %>% 
  filter(METRIC == "NO.LISTS") %>% 
  distinct(DISTRICT.NAME, geometry, PROP.CHANGE1, PROP.CHANGE2) %>% 
  # pivoting longer for facetting
  pivot_longer(cols = c("PROP.CHANGE1", "PROP.CHANGE2"),
               names_to = "METRIC", values_to = "VALUE") %>% 
  mutate(T.CODE = case_when(str_detect(METRIC, "1") ~ "T1",
                            str_detect(METRIC, "2") ~ "T2")) %>% 
  left_join(transition_names)
...
s_spread_distprop <- ggplot(data_spread3) +
  ...
  facet_wrap(~ T.LABEL, ncol = 1, strip.position = "left") +
...
# even the diverging colour scale with midpoint is not enough for a range of values from 0 till 100 on the positive side but just 0 to -1 on the negative.
# this map doesn't tell us anything
...
s_spread_change <- ggplot(data_spread4, aes(x = T.LABEL, y = VALUE)) + 
  geom_point(size = 4, position = position_dodge(0.5)) +
  geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                size = 1.75, width = 0.08, position = position_dodge(0.5)) +
  # scale_y_continuous(limits = c(1.1, 2.3), breaks = seq(1, 3, 0.2)) +
  labs(title = "Net proportional change in birding effort (no. of lists)\nacross districts in transition periods",
       x = "Transition", y = "Proportional change") 

ggsave("03_wrap_figs/10c_s_spread_propchange.png", 
       plot = s_spread_change,
       dpi = 300, width = 6, height = 9, units = "in")
