...
dists_sf_ebd <- data_spat %>% 
  distinct(CELL.ID, COUNTY.CODE, DISTRICT.NAME, STATE.NAME) %>% 
  filter(!is.na(CELL.ID), !is.na(COUNTY.CODE), !is.na(DISTRICT.NAME), !is.na(STATE.NAME)) %>% 
...
temp01 <- data_slice_G %>% 
  inner_join(grid_dist1, by = c("CELL.ID")) 

temp0 <- data_slice_G %>% 
  inner_join(grid_dist, by = c("CELL.ID"))
...
temp2 <- temp0 %>% 
  # average gridcov across two DUR years
  group_by(M.YEAR, STATE.NAME, DISTRICT.NAME, NO.CELLS.TOT, CELL.ID) %>% 
...
temp3 <- temp01 %>% 
  ...
  group_by(COVID, STATE.NAME, DISTRICT.NAME) %>% 
  dplyr::summarise(NO.LISTS = n_distinct(GROUP.ID)) %>% 
  ungroup() %>% 
  # only considering districts covered at least once in the four years
  right_join(dist_cons1 %>% distinct(STATE.NAME, DISTRICT.NAME)) %>% 
  group_by(STATE.NAME, DISTRICT.NAME) %>% 
  complete(COVID, 
...
data_spread1 <- data_spread0 %>% 
  ...
  distinct(DISTRICT.NAME, DISTRICT.GEOM, RAW.CHANGE1, RAW.CHANGE2) %>% 
...
data_spread3a <- data_spread0 %>% 
  filter(METRIC == "NO.LISTS") %>% 
  distinct(DISTRICT.NAME, DISTRICT.GEOM, PROP.CHANGE1a, PROP.CHANGE1b) %>% 
  # pivoting longer for facetting
...
data_spread3b <- data_spread0 %>% 
  filter(METRIC == "NO.LISTS") %>% 
  distinct(DISTRICT.NAME, DISTRICT.GEOM, PROP.CHANGE2a, PROP.CHANGE2b) %>% 
  # pivoting longer for facetting
...
data_spread4 <- data_spread0 %>% 
  ...
  distinct(METRIC, DISTRICT.NAME, DISTRICT.GEOM, RAW.CHANGE1, RAW.CHANGE2) %>% 
  # pivoting longer for facetting
  pivot_longer(cols = c("RAW.CHANGE1", "RAW.CHANGE2"),
...
data_spread6 <- data_spread0 %>% 
  filter(METRIC == "GRID.COV" | METRIC == "GRID.COV.T") %>% 
  distinct(METRIC, DISTRICT.NAME, DISTRICT.GEOM, 
...
save(temp1, temp2, temp3, temp4, temp5, temp6, temp7,
     file = glue("00_outputs/{anal_name}.RData"))
