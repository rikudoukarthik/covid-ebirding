...
data_spread0 <- temp0 %>% 
  ...
  mutate(RAW.CHANGE1 = s_spread_change(BEF, DUR),
         ...
         PROP.CHANGE1a = s_spread_propchange(BEF, DUR),
         PROP.CHANGE1b = s_spread_propchange(DUR, AFT),
         PROP.CHANGE2a = s_spread_propchange(DUR, BEF),
         PROP.CHANGE2b = s_spread_propchange(AFT, DUR)) %>% 
  left_join(grid_dist)
...
s_spread_map <- ggplot(data_spread1) +
  ...
  scale_fill_stepsn(colours = c(RColorBrewer::brewer.pal(n = 7, name = "RdBu")[1],
                                RColorBrewer::brewer.pal(n = 7, name = "RdBu")[4],
                                RColorBrewer::brewer.pal(n = 7, name = "RdBu")[7]),
                    breaks = c(-2500, -1000, -100, 0, 100, 2500, 5000),
                    values = scales::rescale(c(-4134, 0, 7848)), 
                    limits = c(-4134, 7848),
                    na.value = "#e2b33c") +
  labs(x = "Longitude", y = "Latitude",
       fill = "Difference in\nno. of lists",
       title = "Change in no. of lists per district in transition periods",
       subtitle = "Gold: districts in which both periods of the transition had zero lists.\nGrey: districts not considered in analysis.")
...
s_spread_mapprop <- (ggplot(data_spread3a) +
  ...
  labs(x = "Longitude", y = "Latitude",
       fill = "Proportional change")) +
  plot_layout(guides = "collect") +
  plot_annotation(title = "Proportional declines in no. of lists per district in transition periods",
...
s_spread_gridcov_map <- ggplot(data_spread4) +
  ...
  labs(x = "Longitude", y = "Latitude",
       ...
       subtitle = "Gold: districts in which both periods of the transition had zero grid coverage.\nGrey: districts not considered in analysis.")
