...
tictoc::tic(glue("LMM for India"))
model_a <- lmer(LENGTH ~ M.YEAR + M.YEAR:MONTH + (1|CELL.ID),
                data = data_a, 
                control = lmerControl(optimizer = "bobyqa")) 
tictoc::toc() # 38 sec
...
for (i in 1:4) { # for each state
  ...
  tictoc::tic(glue("LMM for {anal_states[, i]}"))
  model_b <- lmer(LENGTH ~ M.YEAR + M.YEAR:MONTH + (1|CELL.ID),
                   data = data_b, 
                   control = lmerControl(optimizer = "bobyqa")) 
  tictoc::toc() 
...
tictoc::tic(glue("GLMM for India"))
model_a <- glmer(URBAN ~ M.YEAR + M.YEAR:MONTH + (1|CELL.ID),
                 data = data_a, family = binomial(link = "cloglog"),
                 nAGQ = 0, control = glmerControl(optimizer = "bobyqa")) 
...
# them into one polygon
  ...
  dplyr::summarise() # dplyr-sf magic :) 
...
temp1 <- data_spread1 %>% st_drop_geometry() %>% dplyr::select(-geometry)
temp2 <- data_spread2
temp3 <- data_spread3a %>% st_drop_geometry() %>% dplyr::select(-geometry)
temp4 <- data_spread3b %>% st_drop_geometry() %>% dplyr::select(-geometry)
temp5 <- data_spread4 %>% st_drop_geometry() %>% dplyr::select(-geometry)
temp6 <- data_spread5 
temp7 <- data_spread6 %>% st_drop_geometry() %>% dplyr::select(-geometry)


save(grid_dist_base0, temp1, temp2, temp3, temp4, temp5, temp6, temp7, indiamap,
     file = glue("00_outputs/{anal_name}.RData"))
