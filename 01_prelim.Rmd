...
```{r s_spread_LULC_MODIS, cache=TRUE, message=FALSE, eval=FALSE}
...
rm(list = setdiff(ls(), "data0"), pos = .GlobalEnv)
...
temp2 <- data_LULC %>% 
  ...
  filter(URBAN == 1) %>% 
  group_by(COVID, MONTH, STATE) %>% 
  summarise(NO.ULISTS = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>%
  ungroup() %>% 
...
urban_nw <- urban_sw %>% 
  group_by(COVID, YEAR, MONTH) %>% 
  summarise(PROP.ULISTS = mean(PROP.ULISTS),
            PROP.SE = sqrt(sum((PROP.SE)^2))/n(),
            PROP.CI.L = PROP.ULISTS - 1.96*PROP.SE,
            PROP.CI.U = PROP.ULISTS + 1.96*PROP.SE)
