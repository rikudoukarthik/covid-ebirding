...
```{r bootSE_fun, include=FALSE, cache=TRUE}
...
boot_se = function(x, fn = median, B = 1000) {
  1:B %>%
...
nl_nw <- nl_sw %>% ungroup() %>% 
  ...
  summarise(NO.LISTS = sum(NO.LISTS),
            NO.SLISTS = sum(NO.SLISTS),
            PROP.SLISTS = round(mean(PROP.SLISTS), 2),
...
ggplot(nl_nw, aes(MONTH, PROP.SLISTS, colour = COVID)) + 
  ...
  geom_errorbar(aes(x = MONTH, ymin = PROP.CI.L, ymax = PROP.CI.U), 
...
ggplot(filter(nl_sw, STATE == "Kerala"), 
  ...
  geom_errorbar(aes(x = MONTH, ymin = PROP.CI.L, ymax = PROP.CI.U), 
                size = 1.25, width = 0.4, position = position_dodge(0.5)) +
  scale_colour_manual(values = covid_palette) +
  labs(title = "Change in monthly group birding with pandemic in Kerala",
       x = "Month", y = "Proportion of group birding lists")
...
ggplot(filter(nl_sw, STATE == "Karnataka"), 
  ...
  geom_errorbar(aes(x = MONTH, ymin = PROP.CI.L, ymax = PROP.CI.U), 
                size = 1.25, width = 0.4, position = position_dodge(0.5)) +
  scale_colour_manual(values = covid_palette) +
  labs(title = "Change in monthly group birding with pandemic in Karnataka",
       x = "Month", y = "Proportion of group birding lists")
...
ggplot(filter(nl_sw, STATE == "Gujarat"), 
  ...
  geom_errorbar(aes(x = MONTH, ymin = PROP.CI.L, ymax = PROP.CI.U), 
                size = 1.25, width = 0.4, position = position_dodge(0.5)) +
  scale_colour_manual(values = covid_palette) +
  labs(title = "Change in monthly group birding with pandemic in Gujarat",
       x = "Month", y = "Proportion of group birding lists")
...
ggplot(filter(nl_sw, STATE == "Maharashtra"), 
  ...
  geom_errorbar(aes(x = MONTH, ymin = PROP.CI.L, ymax = PROP.CI.U), 
                size = 1.25, width = 0.4, position = position_dodge(0.5)) +
  scale_colour_manual(values = covid_palette) +
  labs(title = "Change in monthly group birding with pandemic in Maharashtra",
       x = "Month", y = "Proportion of group birding lists")
...
nl_po_sw <- nl_po_sw %>% ungroup() %>% 
  group_by(COVID, YEAR, MONTH, STATE) %>% 
  summarise(PROP.SLISTS = round(mean(PROP.SLISTS), 2),
            PROP.SE = sqrt(sum((PROP.SE)^2))/n(),
            PROP.CI.L = PROP.SLISTS - 1.96*PROP.SE,
            PROP.CI.U = PROP.SLISTS + 1.96*PROP.SE)
...
ggplot(filter(nl_po_sw, STATE == "Kerala"), 
  ...
  geom_errorbar(aes(x = MONTH, ymin = PROP.CI.L, ymax = PROP.CI.U), 
                size = 1.25, width = 0.4, position = position_dodge(0.5)) +
  scale_colour_manual(values = covid_palette) +
  labs(title = "Change in monthly group birding per observer with pandemic in Kerala",
       x = "Month", y = "Proportion of group birding lists per observer")
...
ggplot(filter(nl_po_sw, STATE == "Karnataka"), 
  ...
  geom_errorbar(aes(x = MONTH, ymin = PROP.CI.L, ymax = PROP.CI.U), 
                size = 1.25, width = 0.4, position = position_dodge(0.5)) +
  scale_colour_manual(values = covid_palette) +
  labs(title = "Change in monthly group birding per observer with pandemic in Karnataka",
       x = "Month", y = "Proportion of group birding lists per observer")
...
ggplot(filter(nl_po_sw, STATE == "Gujarat"), 
  ...
  geom_errorbar(aes(x = MONTH, ymin = PROP.CI.L, ymax = PROP.CI.U), 
                size = 1.25, width = 0.4, position = position_dodge(0.5)) +
  scale_colour_manual(values = covid_palette) +
  labs(title = "Change in monthly group birding per observer with pandemic in Gujarat",
       x = "Month", y = "Proportion of group birding lists per observer")
...
ggplot(filter(nl_po_sw, STATE == "Maharashtra"), 
  ...
  geom_errorbar(aes(x = MONTH, ymin = PROP.CI.L, ymax = PROP.CI.U), 
                size = 1.25, width = 0.4, position = position_dodge(0.5)) +
  scale_colour_manual(values = covid_palette) +
  labs(title = "Change in monthly group birding per observer with pandemic in Maharashtra",
       x = "Month", y = "Proportion of group birding lists per observer")
...
fidel_sw <- data_sub %>% 
  ...
  ungroup() %>% 
  group_by(COVID, YEAR, MONTH, STATE) %>% 
  summarise(NO.SE = boot_se(NO.SITES, mean),
...
(ggplot(fidel_nw, aes(MONTH, NO.SITES, colour = COVID)) + 
    ...
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(x = MONTH, ymin = NO.CI.L, ymax = NO.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
...
time_sw <- data_sub %>% 
  ...
  group_by(COVID, YEAR, MONTH, STATE) %>% 
  summarise(B.TIME.SE = boot_se(B.TIME, mean),
            B.TIME = mean(B.TIME),
...
(ggplot(time_nw, aes(MONTH, B.TIME, colour = COVID)) + 
  scale_x_continuous(breaks = 1:12) + 
  geom_point(size = 3, position = position_dodge(0.5)) +
  geom_errorbar(aes(x = MONTH, ymin = B.TIME.CI.L, ymax = B.TIME.CI.U), 
                size = 1.25, width = 0.4, position = position_dodge(0.5)) +
  scale_colour_manual(values = covid_palette) +
...
hot_sw <- left_join(temp1, temp2)
rm(list = c("temp1","temp2"))

hot_sw <- hot_sw %>% 
  # filling zeroes for states
  complete(COVID, MONTH, STATE, 
           fill = list(NO.LISTS = 0, NO.HLISTS = 0)) %>% 
  mutate(YEAR = as.numeric(
    paste0("20", unlist(strsplit(as.character(COVID), "_"))[2]))) %>% 
  # removing incorrect COVID-YEAR combos produced by complete()
  filter(!(COVID == "DUR_21" & MONTH %in% 9:12),
         !(COVID == "AFT_21" & MONTH %in% 1:8)) %>% 
  # calculating prop. and SE
  group_by(COVID, YEAR, MONTH, STATE) %>% 
  mutate(PROP.HLISTS = round(NO.HLISTS/NO.LISTS, 2),
...
hot_nw <- hot_sw %>% ungroup() %>% 
  ...
  group_by(COVID, YEAR, MONTH) %>% 
  summarise(NO.LISTS = sum(NO.LISTS),
            NO.HLISTS = sum(NO.HLISTS),
            PROP.HLISTS = round(mean(PROP.HLISTS), 2),
            PROP.SE = sqrt(sum((PROP.SE)^2))/n(),
            PROP.CI.L = PROP.HLISTS - 1.96*PROP.SE,
            PROP.CI.U = PROP.HLISTS + 1.96*PROP.SE)
...
prot_sw <- left_join(temp1, temp2)
# rm(list = c("temp1","temp2"))

prot_sw <- prot_sw %>% 
  # filling zeroes for states
  complete(COVID, MONTH, STATE, 
           fill = list(NO.LISTS = 0, NO.TLISTS = 0)) %>% 
  mutate(YEAR = as.numeric(
    paste0("20", unlist(strsplit(as.character(COVID), "_"))[2]))) %>% 
  # removing incorrect COVID-YEAR combos produced by complete()
  filter(!(COVID == "DUR_21" & MONTH %in% 9:12),
  ...
  # calculating prop. and SE
  group_by(COVID, YEAR, MONTH, STATE) %>% 
  mutate(PROP.TLISTS = round(NO.TLISTS/NO.LISTS, 2),
...
prot_nw <- prot_sw %>% ungroup() %>% 
  ...
  group_by(COVID, YEAR, MONTH) %>% 
  summarise(NO.LISTS = sum(NO.LISTS),
            NO.TLISTS = sum(NO.TLISTS),
            PROP.TLISTS = round(mean(PROP.TLISTS), 2),
            PROP.SE = sqrt(sum((PROP.SE)^2))/n(),
            PROP.CI.L = PROP.TLISTS - 1.96*PROP.SE,
            PROP.CI.U = PROP.TLISTS + 1.96*PROP.SE)
...
dist_sw <- data_sub %>% 
  ...
  summarise(DIST = mean(EFFORT.DISTANCE.KM),
            DIST.SE = boot_se(EFFORT.DISTANCE.KM, mean),
...
(ggplot(dist_nw, aes(MONTH, DIST, colour = COVID)) + 
    ...
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(x = MONTH, ymin = DIST.CI.L, ymax = DIST.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "Average distance travelled per checklist at national level",
...
dur_sw <- data_sub %>% 
  ...
  summarise(DURATION = mean(DURATION.MINUTES),
            DURATION.SE = boot_se(DURATION.MINUTES, mean),
...
(ggplot(dur_nw, aes(MONTH, DURATION, colour = COVID)) + 
    ...
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(x = MONTH, ymin = DURATION.CI.L, ymax = DURATION.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
...
```{r t_spread_4, cache=TRUE, message=FALSE}
...
```{r t_spread_5, cache=TRUE, message=FALSE}
...
# CORRECT THE PROPORTION FORMULA!
