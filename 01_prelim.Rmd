...
x <- raster("data/in_LULC_USGS/MCD12Q1.A2019001.h24v05.006.2020212132505.hdf")

# list all the hdf files 
files <- list.files(path = "data/in_LULC_USGS/", pattern = paste0("*.hdf"), 
...
for (i in (files)) {
  ...
  r2 <- gdal_translate(sds[1], dst_dataset = paste0(i, ".tif"))}

rastlist <- list.files(path = "data/in_LULC_USGS/", pattern = ".tif",
                       all.files = TRUE, full.names = TRUE)
allrast <- merge(rastlist)
allrast_b <- brick(allrast)


allrasters <- lapply(rastlist, raster)
# https://stackoverflow.com/a/16007522/13000254
mergerast <- do.call(merge, allrasters)

plot(mergerast)

load("data/maps.RData")
mask <- mask(allrasters, indiamap)

merged <- merge(allrasters)
```
