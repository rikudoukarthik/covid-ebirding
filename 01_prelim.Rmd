...
length_sw <- data_sub %>% 
  ...
  ungroup() %>% 
  group_by(COVID, YEAR, MONTH, STATE) %>% 
  summarise(LENGTH.SE = boot_se(LENGTH, mean),
            LENGTH = mean(LENGTH),
...
temp1 <- data_sub %>% 
  ...
  summarise(NO.LISTS = n_distinct(SAMPLING.EVENT.IDENTIFIER)) %>%
  ungroup() %>% 
  complete(COVID, MONTH, STATE, fill = list(NO.LISTS = 0)) %>% 
  group_by(COVID) %>% 
  mutate(YEAR = as.numeric(
    paste0("20", unlist(strsplit(as.character(COVID), "_"))[2]))) %>% 
  # removing incorrect COVID-YEAR combos produced by complete()
...
temp2 <- data_sub %>% 
  ...
  group_by(COVID, YEAR, MONTH, STATE, DAY.W) %>% 
...
temp3 <- data_sub %>% 
  ...
  group_by(COVID, YEAR, MONTH, STATE, TIME.D) %>% 
...
t_dow_sw <- temp1 %>% 
  ...
  summarise(NO.LISTS = mean(NO.LISTS),
            DAY.LISTS = mean(DAY.LISTS),
