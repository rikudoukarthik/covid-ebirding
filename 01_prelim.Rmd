...
rast_agg_fn <- function(x, ...){
  if_else(sum(x) >= length(x)*0.25, 1, 0)
}
...
data_LULC <- data0 %>% 
  mutate(URBAN = raster::extract(rast_UNU_G1, cbind(LONGITUDE, LATITUDE)))
...
leafcols <- colorNumeric(c("transparent", "#0C2C84"), values(rast_UNU_d),
