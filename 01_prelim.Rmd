...
data_spread_a <- data_LULC_a %>% 
  group_by(SAMPLING.EVENT.IDENTIFIER) %>% 
  slice(1) %>% 
  ungroup() %>% 
  filter(!is.na(URBAN)) %>% 
  group_by(COVID, YEAR, MONTH) %>% 
  dplyr::summarise(NO.U.SE = boot_se(URBAN, fn = sum),
                   NO.NU.SE = boot_se(NONURBAN, fn = sum),
                   NO.LISTS = n_distinct(SAMPLING.EVENT.IDENTIFIER),
                   NO.ULISTS = sum(URBAN),
                   NO.NULISTS = sum(NONURBAN),
                   NO.U.CI.L = NO.ULISTS - 1.96*NO.U.SE,
                   NO.U.CI.U = NO.ULISTS + 1.96*NO.U.SE,
                   NO.NU.CI.L = NO.NULISTS - 1.96*NO.NU.SE,
                   NO.NU.CI.U = NO.NULISTS + 1.96*NO.NU.SE) %>%
  ungroup() 

data_spread_b <- data_LULC_b %>% 
  group_by(SAMPLING.EVENT.IDENTIFIER) %>% 
  slice(1) %>% 
  ungroup() %>% 
  filter(!is.na(URBAN)) %>% 
...
((ggplot(data_spread_a, aes(MONTH, NO.ULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = NO.U.CI.L, ymax = NO.U.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "Urban birding effort (1kmx1km)",
         x = "Month", y = "No. of lists")) /
  (ggplot(data_spread_a, aes(MONTH, NO.NULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = NO.NU.CI.L, ymax = NO.NU.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "Non-urban birding effort (1kmx1km)",
         x = "Month", y = "No. of lists")) |
  (ggplot(data_spread_b, aes(MONTH, NO.ULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = NO.U.CI.L, ymax = NO.U.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "Urban birding effort (2kmx2km)",
         x = "Month", y = "No. of lists")) /
  (ggplot(data_spread_b, aes(MONTH, NO.NULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = NO.NU.CI.L, ymax = NO.NU.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "Non-urban birding effort (2kmx2km)",
         x = "Month", y = "No. of lists"))) +
