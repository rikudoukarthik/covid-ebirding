...
datapath <- "data/ebd_IN_relDec-2021.RData"
...
ggsave("01_prelim_figs/01_no_lists.png", plot = no_lists,
...
ggsave("01_prelim_figs/02_no_lists_po.png", plot = no_lists_po,
       width = 8, height = 9, units = "in")
...
ggsave("01_prelim_figs/03_fidelity.png", plot = fidelity,
...
ggsave("01_prelim_figs/04_time.png", plot = time,
       width = 9, height = 9, units = "in")
...
ggsave("01_prelim_figs/05_hotspot.png", plot = hotspot,
       width = 9, height = 9, units = "in")
...
ggsave("01_prelim_figs/06_protocol.png", plot = protocol,
       width = 9, height = 9, units = "in")
...
ggsave("01_prelim_figs/07_distance.png", plot = distance,
       width = 9, height = 9, units = "in")
...
ggsave("01_prelim_figs/08_duration.png", plot = duration,
       width = 9, height = 9, units = "in")
...
ggsave("01_prelim_figs/09_length.png", plot = length,
       width = 9, height = 9, units = "in")
...
```{r s_spread_cover, cache=TRUE, message=FALSE}
...
plot_layout(guides = "collect") -> s_spread_cover

s_spread_cover

ggsave("01_prelim_figs/10_s_spread_cover.png", plot = s_spread_cover,
       width = 9, height = 9, units = "in")


```

```{r s_spread_map, cache=TRUE, message=FALSE}
...
(ggplot() +
  ...
  geom_polygon(data = filter(data_map, COVID == "BEF_19"), 
...
(ggplot() +
  geom_polygon(data = fortify(india), 
               aes(long, lat, group = group),
               fill = NA, colour = "black", lwd = 1) +
  geom_polygon(data = filter(data_map, COVID == "DUR_20"), 
               aes(long, lat, group = group, fill = log(NO.LISTS)),
               colour = NA, lwd = 0.5) +
  scale_fill_distiller(palette = "Reds", direction = 1) +
  labs(title = "DUR_20")) |
(ggplot() +
  geom_polygon(data = fortify(india), 
               aes(long, lat, group = group),
               fill = NA, colour = "black", lwd = 1) +
  geom_polygon(data = filter(data_map, COVID == "AFT_21"), 
               aes(long, lat, group = group, fill = log(NO.LISTS)),
               colour = NA, lwd = 0.5) +
  scale_fill_distiller(palette = "Reds", direction = 1) +
  labs(title = "AFT_21")) +
plot_layout(guides = "collect") -> s_spread_map

s_spread_map

ggsave("01_prelim_figs/11_s_spread_map.png", plot = s_spread_map,
       width = 14, height = 5, units = "in")

rm(data_cov, data_map, data_sp, s_spread_cover, s_spread_map, districtmap,
...
```{r s_spread_LULC_MODIS, cache=TRUE, message=FALSE}

library(raster) # masks dplyr functions, so have used raster::function()
require(terra)
library(gdalUtils)
library(geodata)
...
# reclassifying the raster as urban vs. non-urban (help from:
...
# 2km*2km
rast_UNU_b <- rast_UNU_a %>% 
  raster::aggregate(fact = 2, fun = rast_agg_fn)


# # g1 grid is 25km*25km
# rast_UNU_G1 <- rast_UNU %>% 
#   raster::aggregate(fact = 2, fun = rast_agg_fn) %>%
#   raster::aggregate(fact = 5, fun = rast_agg_fn) %>%
#   raster::aggregate(fact = 5, fun = rast_agg_fn)

# # 4km*4km
# rast_UNU_c <- rast_UNU %>% 
#   raster::aggregate(fact = 2, fun = rast_agg_fn) %>%
#   raster::aggregate(fact = 4, fun = rast_agg_fn)
# 
# # 5km*5km
# rast_UNU_d <- rast_UNU %>% 
#   raster::aggregate(fact = 2, fun = rast_agg_fn) %>%
...
raster::plot(rast_UNU, main = "500mx500m") 
raster::plot(rast_UNU_a, main = "1kmx1km")
raster::plot(rast_UNU_b, main = "2kmx2km")
# raster::plot(rast_UNU_G1, main = "25kmx25km")
# raster::plot(rast_UNU_c, main = "4kmx4km")
# raster::plot(rast_UNU_d, main = "5kmx5km")

save(rast_UNU, file = "data/rast_UNU.RData")
save(rast_UNU_a, file = "data/rast_UNU_a.RData")
save(rast_UNU_b, file = "data/rast_UNU_b.RData")
# save(rast_UNU_G1, file = "data/rast_UNU_G1.RData")
# save(rast_UNU_c, file = "data/rast_UNU_c.RData")
# save(rast_UNU_d, file = "data/rast_UNU_d.RData")
...
leafcols <- colorFactor(c(NA, "#0C2C84"), values(rast_UNU_a),
                         na.color = "#ffba00")
...
leaflet() %>% 
  addTiles() %>% 
  addRasterImage(rast_UNU_b, 
                 method = "ngb", # nearest neighbour
                 colors = leafcols,
                 opacity = 0.7,
                 maxBytes = 150 * 1024 * 1024) %>% 
  addLegend(pal = leafcols, values = values(rast_UNU_b))

leaflet() %>% 
  addTiles() %>% 
  addRasterImage(rast_UNU_a, 
                 method = "ngb", # nearest neighbour
                 colors = leafcols,
                 opacity = 0.7,
                 maxBytes = 150 * 1024 * 1024) %>% 
  addLegend(pal = leafcols, values = values(rast_UNU_a))

# 
# leaflet() %>% 
#   addTiles() %>% 
#   addRasterImage(rast_UNU_G1, 
#                  colors = leafcols,
#                  opacity = 0.7,
#                  maxBytes = 10 * 1024 * 1024) %>% 
#   addLegend(pal = leafcols, values = values(rast_UNU_G1))
# 
# leaflet() %>% 
#   addTiles() %>% 
#   addRasterImage(rast_UNU_d, 
#                  colors = leafcols,
#                  opacity = 0.7,
#                  maxBytes = 10 * 1024 * 1024) %>% 
#   addLegend(pal = leafcols, values = values(rast_UNU_d))
# 
# leaflet() %>% 
#   addTiles() %>% 
#   addRasterImage(rast_UNU_c, 
#                  colors = leafcols,
#                  opacity = 0.7,
#                  maxBytes = 100 * 1024 * 1024) %>% 
#   addLegend(pal = leafcols, values = values(rast_UNU_c))



```

* From this, it looks like only the month of April (or Mar-May) was affected by the pandemic in 2020, while 2021 saw some increased urban birding in October and December.

```{r s_spread_UNU, cache=TRUE, message=FALSE}
...
# load("data/rast_UNU_a.RData")
# load("data/rast_UNU_b.RData")



# adding extracted land cover values at eBird data points
# Pakistan-administered Kashmir lists produce NA for URBAN
dataUNU_a <- data0 %>% 
  mutate(URBAN = raster::extract(rast_UNU_a, cbind(LONGITUDE, LATITUDE)),
         NONURBAN = if_else(URBAN == 1, 0, 1),
         CELL.ID = cellFromXY(rast_UNU_a, cbind(LONGITUDE, LATITUDE))) 


dataUNU_b <- data0 %>% 
  mutate(URBAN = raster::extract(rast_UNU_b, cbind(LONGITUDE, LATITUDE)),
         NONURBAN = if_else(URBAN == 1, 0, 1),
         CELL.ID = cellFromXY(rast_UNU_b, cbind(LONGITUDE, LATITUDE)))  


# searching for effect of pandemic
...
temp2 <- dataUNU_a %>% 
  ...
  ungroup() %>% 
  complete(COVID, MONTH, STATE, fill = list(NO.ULISTS = 0))
...
(ggplot(urban_nw_a, aes(MONTH, PROP.ULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = PROP.CI.L, ymax = PROP.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "Change in proportion of urban lists at national level",
         x = "Month", y = "Proportion of urban lists")) /
((ggplot(filter(urban_sw_a, STATE == "Kerala"), 
    ...
    labs(title = "Kerala",
         x = "Month", y = "Proportion of urban lists")) |
(ggplot(filter(urban_sw_a, STATE == "Karnataka"), 
         aes(MONTH, PROP.ULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = PROP.CI.L, ymax = PROP.CI.U), 
    ...
    labs(title = "Karnataka",
         x = "Month", y = "Proportion of urban lists"))) /
((ggplot(filter(urban_sw_a, STATE == "Gujarat"), 
         aes(MONTH, PROP.ULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = PROP.CI.L, ymax = PROP.CI.U), 
    ...
    labs(title = "Gujarat",
         x = "Month", y = "Proportion of urban lists")) |
(ggplot(filter(urban_sw_a, STATE == "Maharashtra"), 
         aes(MONTH, PROP.ULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = PROP.CI.L, ymax = PROP.CI.U), 
    ...
    labs(title = "Maharashtra",
         x = "Month", y = "Proportion of urban lists"))) +
plot_layout(guides= "collect") -> s_spread_LULC_MODIS_a
...
(ggplot(urban_nw_b, aes(MONTH, PROP.ULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = PROP.CI.L, ymax = PROP.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    ...
    labs(title = "Change in proportion of urban lists at national level",
         x = "Month", y = "Proportion of urban lists")) /
((ggplot(filter(urban_sw_b, STATE == "Kerala"), 
         aes(MONTH, PROP.ULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = PROP.CI.L, ymax = PROP.CI.U), 
    ...
    labs(title = "Kerala",
         x = "Month", y = "Proportion of urban lists")) |
(ggplot(filter(urban_sw_b, STATE == "Karnataka"), 
         aes(MONTH, PROP.ULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = PROP.CI.L, ymax = PROP.CI.U), 
    ...
    labs(title = "Karnataka",
         x = "Month", y = "Proportion of urban lists"))) /
((ggplot(filter(urban_sw_b, STATE == "Gujarat"), 
         aes(MONTH, PROP.ULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = PROP.CI.L, ymax = PROP.CI.U), 
    ...
    labs(title = "Gujarat",
         x = "Month", y = "Proportion of urban lists")) |
(ggplot(filter(urban_sw_b, STATE == "Maharashtra"), 
         aes(MONTH, PROP.ULISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = PROP.CI.L, ymax = PROP.CI.U), 
    ...
    labs(title = "Maharashtra",
         x = "Month", y = "Proportion of urban lists"))) +
plot_layout(guides= "collect") -> s_spread_LULC_MODIS_b

ggsave("01_prelim_figs/12_s_spread_LULC_MODIS_a.png", 
       plot = s_spread_LULC_MODIS_a,
       width = 9, height = 9, units = "in")
ggsave("01_prelim_figs/12_s_spread_LULC_MODIS_b.png", 
       plot = s_spread_LULC_MODIS_b,
       width = 9, height = 9, units = "in")
...
# s_spread_LULC_MODIS
# 
# ggsave("01_prelim_figs/12_s_spread_LULC_MODIS.png", 
#        plot = s_spread_LULC_MODIS,
#        width = 9, height = 9, units = "in")

rm(data_LULC, rast, rast_UNU, rast_UNU_G1, reclass, categs,
...
```{r s_spread_UNU_cover, cache=TRUE, message=FALSE}

data_spread_a <- dataUNU_a %>% 
  group_by(SAMPLING.EVENT.IDENTIFIER) %>% 
  slice(1) %>% 
  ungroup() %>% 
  filter(!is.na(URBAN)) %>% 
  group_by(COVID, YEAR, MONTH) %>% 
  dplyr::summarise(U.SE = boot_se(URBAN, fn = sum),
                   NU.SE = boot_se(NONURBAN, fn = sum),
                   LISTS = n_distinct(SAMPLING.EVENT.IDENTIFIER),
                   U.LISTS = sum(URBAN),
                   NU.LISTS = sum(NONURBAN),
                   U.CI.L = U.LISTS - 1.96*U.SE,
                   U.CI.U = U.LISTS + 1.96*U.SE,
                   NU.CI.L = NU.LISTS - 1.96*NU.SE,
                   NU.CI.U = NU.LISTS + 1.96*NU.SE) %>%
  ungroup() 

data_spread_b <- dataUNU_b %>% 
  group_by(SAMPLING.EVENT.IDENTIFIER) %>% 
  slice(1) %>% 
  ungroup() %>% 
  filter(!is.na(URBAN)) %>% 
...
((ggplot(data_spread_a, aes(MONTH, U.LISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = U.CI.L, ymax = U.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "1kmx1km (urban)",
         x = "Month", y = "No. of lists")) /
  (ggplot(data_spread_a, aes(MONTH, NU.LISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = NU.CI.L, ymax = NU.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "1kmx1km (non-urban)",
         x = "Month", y = "No. of lists")) |
  (ggplot(data_spread_b, aes(MONTH, U.LISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = U.CI.L, ymax = U.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "2kmx2km (urban)",
         x = "Month", y = "No. of lists")) /
  (ggplot(data_spread_b, aes(MONTH, NU.LISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = NU.CI.L, ymax = NU.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "2kmx2km (non-urban)",
         x = "Month", y = "No. of lists"))) +
  plot_layout(guides = "collect") +
  plot_annotation(
    title = "Birding effort in lists at two resolutions"
    ) -> s_spread_UNU_cover_lists

ggsave("01_prelim_figs/12_s_spread_UNU_cover_lists.png",
       plot = s_spread_UNU_cover_lists,
...
cells_UNU_a <- dataUNU_a %>% 
  group_by(SAMPLING.EVENT.IDENTIFIER) %>% slice(1) %>% ungroup() %>% 
  # below removes any cell with <1 complete list
  filter(ALL.SPECIES.REPORTED == 1) %>% 
  group_by(COVID, YEAR, MONTH, CELL.ID) %>% slice(1) %>% ungroup() %>% 
  filter(!is.na(URBAN)) %>% 
  group_by(COVID, YEAR, MONTH) %>% 
  dplyr::summarise(U.SE = boot_se(URBAN, fn = sum),
                   NU.SE = boot_se(NONURBAN, fn = sum),
                   U.CELLS = sum(URBAN),
                   NU.CELLS = sum(NONURBAN),
                   U.CI.L = U.CELLS - 1.96*U.SE,
                   U.CI.U = U.CELLS + 1.96*U.SE,
                   NU.CI.L = NU.CELLS - 1.96*NU.SE,
                   NU.CI.U = NU.CELLS + 1.96*NU.SE) %>%
  ungroup() 

cells_UNU_b <- dataUNU_b %>% 
  group_by(SAMPLING.EVENT.IDENTIFIER) %>% slice(1) %>% ungroup() %>% 
  # below removes any cell with <1 complete list
  filter(ALL.SPECIES.REPORTED == 1) %>% 
  group_by(COVID, YEAR, MONTH, CELL.ID) %>% slice(1) %>% ungroup() %>% 
...
((ggplot(cells_UNU_a, aes(MONTH, U.CELLS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = U.CI.L, ymax = U.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    scale_colour_manual(values = covid_palette) +
    labs(title = "1kmx1km (urban)",
         x = "Month", y = "No. of cells")) /
  (ggplot(cells_UNU_a, aes(MONTH, NU.CELLS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = NU.CI.L, ymax = NU.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    ...
    labs(title = "1kmx1km (non-urban)",
         x = "Month", y = "No. of cells")) |
  (ggplot(cells_UNU_b, aes(MONTH, U.CELLS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = U.CI.L, ymax = U.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    ...
    labs(title = "2kmx2km (urban)",
         x = "Month", y = "No. of cells")) /
  (ggplot(cells_UNU_b, aes(MONTH, NU.CELLS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + 
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = NU.CI.L, ymax = NU.CI.U), 
                  size = 1.25, width = 0.4, position = position_dodge(0.5)) +
    ...
    labs(title = "2kmx2km (non-urban)",
         x = "Month", y = "No. of cells"))) +
  plot_layout(guides = "collect") +
  plot_annotation(title = "Birding effort in grid cells at two resolutions",
                  subtitle = "Y axis show number of cells with at least one complete list") -> s_spread_UNU_cover_cells


ggsave("01_prelim_figs/12_s_spread_UNU_cover_cells.png",
       plot = s_spread_UNU_cover_cells,
       width = 11, height = 7, units = "in")
...
ggsave("01_prelim_figs/13_t_spread2.png", 
       plot = t_spread2,
...
ggsave("01_prelim_figs/14_t_spread3.png", 
       plot = t_spread3,
       width = 11, height = 9, units = "in")
...
ggsave("01_prelim_figs/15_media.png", plot = media,
       width = 9, height = 9, units = "in")
...
ggsave("01_prelim_figs/16_new.png", plot = new,
       width = 9, height = 9, units = "in")
