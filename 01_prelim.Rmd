...
set.seed(420)
x <- nl_nw %>% group_by(COVID, MONTH) %>% 
  summarise(BOOT.LISTS = mean(sample(NO.LISTS, size = 999, replace = T)),
            BOOT.LISTS.CI.L = quantile(sample(NO.LISTS, size = 999, replace = T),
                                       probs = 0.025),
            BOOT.LISTS.CI.U = quantile(sample(NO.LISTS, size = 999, replace = T),
                                       probs = 0.975))
...
notes_star <- sample(NO.LISTS, N, replace = T)
