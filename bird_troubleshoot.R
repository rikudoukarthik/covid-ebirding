model_test <- function(birds_pred0,
                       data_mtype,
                       data_occ,
                       cur_i, cur_m) {

  birds_pred0_b <- birds_pred0 %>% 
    filter(COMMON.NAME == unique(birds_pred0$COMMON.NAME)[cur_i]) %>% 
  ...
  tictoc::tic(glue("GLMM for months type {cur_m}, {unique(birds_pred0$COMMON.NAME)[cur_i]}"))
  ...
  tictoc::tic(glue("Bootstrapped predictions for months type {cur_m}, {unique(birds_pred0$COMMON.NAME)[cur_i]}"))
