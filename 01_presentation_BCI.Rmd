...
.pull-right[
.right[
![:scale 70%](pics/woes.png)  
<small>Instagram [@birding.memes](https://www.instagram.com/birding.memes/)</small>
]
]
...
ggplot(filter(vax, state == "India"), aes(date, dose2)) + geom_line() +
  scale_y_continuous(labels = scales::label_comma()) +
  ggtitle("Second vaccine doses administered") +
  xlab("Date") + ylab("Second doses administered") +
  theme(axis.text.y = element_text(angle = 90),
        axis.title.y = element_text(vjust = 3))
```
]
...
.pull-left[
```{r no_lists_1, cache=TRUE, message=FALSE, echo=FALSE, fig.height=5}
load("data/data_sub.RData")
...
.pull-right[
```{r no_lists_2, cache=TRUE, message=FALSE, echo=FALSE, fig.height=5}
...
```{r no_lists_po_1, cache=TRUE, message=FALSE, echo=FALSE, fig.height=4, fig.width=9}
temp1 <- data_sub %>% 
  group_by(SAMPLING.EVENT.IDENTIFIER) %>% 
  slice(1) %>% ungroup() %>% 
...
.pull-left[
```{r no_lists_po_2, cache=TRUE, message=FALSE, echo=FALSE, fig.height=6}
...
.pull-right[
```{r no_lists_po_3, cache=TRUE, message=FALSE, echo=FALSE, fig.height=6}
...
.center[
```{r protocol_1, cache=TRUE, message=FALSE, echo=FALSE, fig.height=4, fig.width=9}
...
.pull-left[
```{r protocol_2, cache=TRUE, message=FALSE, echo=FALSE, fig.height=6}
...
.pull-right[
```{r protocol_3, cache=TRUE, message=FALSE, echo=FALSE, fig.height=6}
...
.pull-left[
```{r distance_1, cache=TRUE, message=FALSE, echo=FALSE, fig.height=5}
...
.pull-right[
```{r distance_2, cache=TRUE, message=FALSE, echo=FALSE, fig.height=5}
...
.pull-left[
```{r distance_3, cache=TRUE, message=FALSE, echo=FALSE, fig.height=5}
...
.pull-right[
```{r distance_4, cache=TRUE, message=FALSE, echo=FALSE, fig.height=5}
...
*A unique location being defined as a $\approx 1 km \times 1 km$ area defined by degree coordinates.
]
...
.pull-left[
```{r fidelity_1, cache=TRUE, message=FALSE, echo=FALSE, fig.height=5}
...
.pull-right[
```{r fidelity_2, cache=TRUE, message=FALSE, echo=FALSE, fig.height=5}
...
.pull-left[
```{r fidelity_3, cache=TRUE, message=FALSE, echo=FALSE, fig.height=5}
...
.pull-right[
```{r fidelity_4, cache=TRUE, message=FALSE, echo=FALSE, fig.height=5}
...
.center[
```{r time, cache=TRUE, message=FALSE, echo=FALSE, fig.height=4, fig.width=9}
...
(ggplot(t_dow_nw, aes(DAYW, PROP.LISTS, colour = COVID)) + 
    scale_x_continuous(breaks = 1:12) + geom_smooth() +
    ggtitle("Days of week (national level)") +
    xlab("Day") + ylab("Proportion of lists")) /
  (ggplot(t_dow_nw, aes(as.factor(DAYW), PROP.LISTS, fill = COVID)) + 
     geom_boxplot() +
     xlab("Day") + ylab("Proportion of lists"))
...
(ggplot(t_tod_nw, aes(TIMED, PROP.LISTS, colour = COVID)) + 
   ...
   ggtitle("Times of day (national level)") +
   xlab("Hour") + ylab("Proportion of lists")) /
  (ggplot(t_tod_nw, aes(as.factor(TIMED), PROP.LISTS, fill = COVID)) + 
     geom_boxplot() +
     xlab("Hour") + ylab("Proportion of lists"))
...
# library(xaringanBuilder)
# build_pdf("01_presentation_BCI.Rmd", partial_slides = T)
```
