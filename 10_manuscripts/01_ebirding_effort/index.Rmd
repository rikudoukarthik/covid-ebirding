...
data0 <- join_metric_composite(base = NULL, 
  ...
  join_metric_composite(data_master[3], "TIME", scale, state) %>% 
  join_metric_composite(data_master[4], "HOTSPOT", scale, state) %>% 
  join_metric_composite(data_master[5], "TRAVELLING", scale, state) %>% 
  join_metric_composite(data_master[6], "DISTANCE", scale, state) %>% 
  join_metric_composite(data_master[7], "DURATION", scale, state) %>% 
  join_metric_composite(data_master[8], "LENGTH", scale, state) %>% 
  join_metric_composite(data_master[9], "URBAN", scale, state) %>% 
  join_metric_composite(data_master[10], "COVER", scale, state) %>% 
  # summarising across metrics and months while propagating error
  group_by(M.YEAR, COVID.MONTH) %>% 
  summarise_mean_and_se(PRED, SE) %>% 
  get_CI_lims()

lab_y <- if (ylab == FALSE) NA else "Data quality metrics<br>(prop. change from t~0~ = 2018\u201319)"
...
grid_sizes_km <- calc_celldim_extremes()
...
data_list <- c("d01_group_po", "d02_fidelity_po", "d03_time_po", "d04_hotspot",
