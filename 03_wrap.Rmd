...
##### Final analysis: statewise GLMMs (but for number not proportion) ####
...
data0 <- data0_MY_slice_G %>% 
  ...
  mutate(DAY.W = wday(OBSERVATION.DATE, 
                      label = T,
                      week_start = getOption("lubridate.week.start", 1))) %>% 
  tidyr::expand(nesting(STATE, M.YEAR, DAY.W)) %>% 
  mutate(PRED = NA,
         STATE = factor(STATE, levels = anal_states[1,])) %>% 
  arrange(STATE)
...
for (i in 1:4) { # for each state
  ...
  data_a <- data0_MY_slice_G %>% 
    ...
    mutate(DAY.W = wday(OBSERVATION.DATE, 
                        label = T,
                        week_start = getOption("lubridate.week.start", 1))) %>% 
    group_by(STATE, M.YEAR, MONTH, DAY.W) %>% 
    dplyr::summarise(LISTS = replace_na(n_distinct(SAMPLING.EVENT.IDENTIFIER)))
  
  tictoc::tic(glue::glue("GLMM for {anal_states[, i]}"))
  model_a <- glmer(LISTS ~ M.YEAR + M.YEAR:DAY.W + (1|MONTH),
                   data = data_a, family = poisson,
                   nAGQ = 0, control = glmerControl(optimizer = "bobyqa"))  
  tictoc::toc()
  

  for (j in 1:n_distinct(data0$M.YEAR)) {
    for (k in 1:n_distinct(data0$DAY.W)) {
      count <- count + 1
      
      data0$PRED[count] = predict(model_a,
                                  data.frame(DAY.W = data0$DAY.W[count],
                                             M.YEAR = data0$M.YEAR[count]),
                                  re.form = NA, type = "response")
    }
  }
...
(ggplot(filter(data0, STATE == anal_states$a), 
       aes(DAY.W, PRED, colour = M.YEAR)) +
  labs(title = anal_states$a,
       x = "Day of week", y = "Predicted number of checklists") +
  geom_point(size = 3, position = position_dodge(0.5)) +
  # geom_errorbar(aes(ymin = CI.L, ymax = CI.U),
  #               size = 1.5, width = 0.2, position = position_dodge(0.5)) +
...
data0 <- data0_MY_slice_G %>% 
  ...
  tidyr::expand(nesting(STATE, M.YEAR, HOUR)) %>% 
  mutate(PRED = NA,
         STATE = factor(STATE, levels = anal_states[1,])) %>% 
  arrange(STATE)
...
for (i in 1:4) { # for each state
  ...
  data_a <- data0_MY_slice_G %>% 
    filter(STATE == anal_states[, i]) %>% 
    mutate(HOUR = factor(HOUR)) %>% 
    group_by(STATE, M.YEAR, MONTH, HOUR) %>% 
    dplyr::summarise(LISTS = replace_na(n_distinct(SAMPLING.EVENT.IDENTIFIER)))
  
  tictoc::tic(glue::glue("GLMM for {anal_states[, i]}"))
  model_a <- glmer(LISTS ~ M.YEAR + M.YEAR:HOUR + (1|MONTH),
                   data = data_a, family = poisson,
                   nAGQ = 0, control = glmerControl(optimizer = "bobyqa"))  
  tictoc::toc()
  ...
  for (j in 1:n_distinct(data0$M.YEAR)) {
    for (k in 1:n_distinct(data0$HOUR)) {
      count <- count + 1
      
      data0$PRED[count] = predict(model_a,
                                  data.frame(HOUR = data0$HOUR[count],
                                             M.YEAR = data0$M.YEAR[count]),
                                  re.form = NA, type = "response")
    }
  }
...
(ggplot(filter(data0, STATE == anal_states$a), 
       aes(HOUR, PRED, colour = M.YEAR)) +
  labs(title = anal_states$a,
       x = "Time of day", y = "Predicted number of checklists") +
  geom_point(size = 3, position = position_dodge(0.5)) +
  # geom_errorbar(aes(ymin = CI.L, ymax = CI.U),
  #               size = 1.5, width = 0.2, position = position_dodge(0.5)) +
  scale_colour_manual(values = covid_palette, name = "Migratory year")) /
...
# full data
data_prolific1 <- data_prolific

# getting median list length (patch-wise) for prediction later
median_length1 <- data_prolific1 %>% 
  distinct(LOCALITY.ID, M.YEAR, MONTH, SAMPLING.EVENT.IDENTIFIER, NO.SP) %>% 
  group_by(LOCALITY.ID, MONTH) %>% 
  summarise(NO.SP.MED = floor(median(NO.SP)))

# dataframe with empty column to populate with looped values
prolific1 <- data_prolific1 %>% 
  group_by(LOCALITY.ID) %>% 
  tidyr::expand(MONTH, M.YEAR, SP.CATEGORY) %>% 
  ungroup() %>% 
  mutate(REP.FREQ.PRED = NA)

count <- 0
for (l in 1:n_distinct(prolific1$LOCALITY.ID)) {
  
  data_loc <- data_prolific1 %>% 
    filter(LOCALITY.ID == unique(prolific1$LOCALITY.ID)[l])
  ...
  for (i in 1:n_distinct(prolific1$MONTH)) {
    
    for (j in 1:n_distinct(prolific1$M.YEAR)) {
      
      for (k in 1:n_distinct(prolific1$SP.CATEGORY)) {
        
        count <- count + 1
        
        prolific1$REP.FREQ.PRED[count] = predict(
          model_spec,
          data.frame(LOCALITY.ID = prolific1$LOCALITY.ID[count],
                     MONTH = prolific1$MONTH[count],
                     M.YEAR = prolific1$M.YEAR[count],
                     SP.CATEGORY = prolific1$SP.CATEGORY[count],
                     NO.SP = median_length1$NO.SP.MED[prolific1$MONTH[count]]),
          re.form = NA, 
          type = "response")
        
        print(glue::glue("Completed predictions for loc. {l}, month {prolific1$MONTH[count]}, migratory year {prolific1$M.YEAR[count]}"))
...
(ggplot(prolific1a, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                  size = 1.5, width = 0.2, position = position_dodge(0.5)) |
  ggplot(prolific1b, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    scale_y_continuous(limits = c(0.1, 0.7)) +
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                   size = 1.5, width = 0.2, position = position_dodge(0.5)) |
  ggplot(prolific1c, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    scale_y_continuous(limits = c(0.1, 0.7)) +
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                  size = 1.5, width = 0.2, position = position_dodge(0.5))) +
  plot_layout(guides = "collect") +
  plot_annotation(title = "Bird species reporting in the pandemic, with constant effort",
                  subtitle = "Predicted reporting frequencies of common urban and rural species, from locations with consistent effort. (One model, visual comparison of months.)") -> b_prolific1

ggsave("03_wrap_figs/b_prolific1.png", b_prolific1,
       width = 21, height = 6, units = "in", dpi = 300)

### modelling different months separately ####

data_prolific2a <- data_prolific %>% filter(MONTH %in% 4:5) 
data_prolific2b <- data_prolific %>% filter(!(MONTH %in% 4:5))

# getting median list length (patch-wise) for prediction later
median_length2a <- data_prolific2a %>% 
  distinct(LOCALITY.ID, M.YEAR, MONTH, SAMPLING.EVENT.IDENTIFIER, NO.SP) %>% 
  group_by(LOCALITY.ID, MONTH) %>% 
  summarise(NO.SP.MED = floor(median(NO.SP)))
median_length2b <- data_prolific2b %>% 
  distinct(LOCALITY.ID, M.YEAR, MONTH, SAMPLING.EVENT.IDENTIFIER, NO.SP) %>% 
  group_by(LOCALITY.ID, MONTH) %>% 
  summarise(NO.SP.MED = floor(median(NO.SP)))

prolific2a <- data_prolific2a %>% 
  group_by(LOCALITY.ID) %>% 
  tidyr::expand(nesting(MONTH), M.YEAR, SP.CATEGORY) %>% 
  ungroup() %>% 
  mutate(REP.FREQ.PRED = NA)

prolific2b <- data_prolific2b %>% 
  group_by(LOCALITY.ID) %>% 
  tidyr::expand(nesting(MONTH), M.YEAR, SP.CATEGORY) %>% 
  ungroup() %>% 
  mutate(REP.FREQ.PRED = NA)
...
count <- 0
for (l in 1:n_distinct(prolific2a$LOCALITY.ID)) {
  
  data_loc <- data_prolific2a %>% 
    filter(LOCALITY.ID == unique(prolific2a$LOCALITY.ID)[l])
  ...
  for (i in 1:n_distinct(prolific2a$MONTH)) {
    
    for (j in 1:n_distinct(prolific2a$M.YEAR)) {
      
      for (k in 1:n_distinct(prolific2a$SP.CATEGORY)) {
        
        count <- count + 1
        
        prolific2a$REP.FREQ.PRED[count] = predict(
          model_spec,
          data.frame(LOCALITY.ID = prolific2a$LOCALITY.ID[count],
                     MONTH = prolific2a$MONTH[count],
                     M.YEAR = prolific2a$M.YEAR[count],
                     NO.SP = median_length2a$NO.SP.MED[prolific2a$MONTH[count]],
                     SP.CATEGORY = prolific2a$SP.CATEGORY[count]),
          re.form = NA, 
          type = "response")
        
        print(glue::glue("Completed predictions for loc. {l}, month {prolific2a$MONTH[count]}, migratory year {prolific2a$M.YEAR[count]}"))
...
count <- 0
for (l in 1:n_distinct(prolific2b$LOCALITY.ID)) {
  
  data_loc <- data_prolific2b %>% 
    filter(LOCALITY.ID == unique(prolific2b$LOCALITY.ID)[l])
  ...
  for (i in 1:n_distinct(prolific2b$MONTH)) {
    
    for (j in 1:n_distinct(prolific2b$M.YEAR)) {
      
      for (k in 1:n_distinct(prolific2b$SP.CATEGORY)) {
        
        count <- count + 1
        
        prolific2b$REP.FREQ.PRED[count] = predict(
          model_spec,
          data.frame(LOCALITY.ID = prolific2b$LOCALITY.ID[count],
                     MONTH = prolific2b$MONTH[count],
                     M.YEAR = prolific2b$M.YEAR[count],
                     NO.SP = median_length2b$NO.SP.MED[prolific2b$MONTH[count]],
                     SP.CATEGORY = prolific2b$SP.CATEGORY[count]),
          re.form = NA, 
          type = "response")
        
        print(glue::glue("Completed predictions for loc. {l}, month {prolific2b$MONTH[count]}, migratory year {prolific2b$M.YEAR[count]}"))
        
      }
    }
  }
} # Janardhan Uppada's patch not converging


# lockdown months
prolific2a <- prolific2a %>% 
  filter(!is.na(REP.FREQ.PRED)) %>% 
  group_by(M.YEAR, SP.CATEGORY) %>% 
  summarise(SE = sd(REP.FREQ.PRED)/sqrt(n()),
            REP.FREQ.PRED = mean(REP.FREQ.PRED),
            CI.L = REP.FREQ.PRED - 1.96*SE,
...
# all months
prolific2c <- prolific1c



(ggplot(prolific2a, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                  size = 1.5, width = 0.2, position = position_dodge(0.5)) |
  ggplot(prolific2b, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    scale_y_continuous(limits = c(0.1, 0.8)) +
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                   size = 1.5, width = 0.2, position = position_dodge(0.5)) |
  ggplot(prolific2c, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    scale_y_continuous(limits = c(0.1, 0.8)) +
  ...
  plot_layout(guides = "collect") +
  plot_annotation(title = "Bird species reporting in the pandemic, with constant effort",
                  subtitle = "Predicted reporting frequencies of common urban and rural species, from locations with consistent effort. (Individual models for comparison of months.)") -> b_prolific2

ggsave("03_wrap_figs/b_prolific2.png", b_prolific2,
       width = 21, height = 6, units = "in", dpi = 300)


#######

prolific2x <- data_prolific2a %>% 
  group_by(LOCALITY.ID) %>% 
  tidyr::expand(nesting(MONTH), M.YEAR, SP.CATEGORY) %>% 
  ungroup() %>% 
  mutate(REP.FREQ.PRED = NA)
...
count <- 0
for (l in 1:n_distinct(prolific2x$LOCALITY.ID)) {
  
  data_loc <- data_prolific2a %>% 
    filter(LOCALITY.ID == unique(prolific2x$LOCALITY.ID)[l])
  ...
  for (i in 1:n_distinct(prolific2x$SP.CATEGORY)) {
    
    for (j in 1:n_distinct(prolific2x$M.YEAR)) {
      
      for (k in 1:n_distinct(prolific2x$MONTH)) {
        
        count <- count + 1
        
        prolific2x$REP.FREQ.PRED[count] = predict(
          model_spec,
          data.frame(LOCALITY.ID = prolific2x$LOCALITY.ID[count],
                     MONTH = prolific2x$MONTH[count],
                     M.YEAR = prolific2x$M.YEAR[count],
                     NO.SP = median_length2a$NO.SP.MED[prolific2x$MONTH[count]],
                     SP.CATEGORY = prolific2x$SP.CATEGORY[count]),
          re.form = NA, 
          type = "response")
        
        print(glue::glue("Completed predictions for loc. {l}, month {prolific2x$MONTH[count]}, migratory year {prolific2x$M.YEAR[count]}"))
...
(ggplot(prolific2x, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    scale_y_continuous(limits = c(0.1, 0.8)) +
  ...
  plot_layout(guides = "collect") +
  plot_annotation(title = "Bird species reporting in the pandemic, with constant effort",
                  subtitle = "Predicted reporting frequencies of common urban and rural species, from locations with consistent effort. (Individual models for comparison of months.)") -> b_prolific2x

ggsave("03_wrap_figs/b_prolific2x.png", b_prolific2x,
       width = 21, height = 6, units = "in", dpi = 300)




x <- data.frame(A = factor(c("head")),
                B = factor(c("abc", "def")),
                C = factor(1:10)) %>% 
  tidyr::expand(A, B, C)
...
count <- 0
for (i in 1:n_distinct(x$A)) {

  for (j in 1:n_distinct(x$B)) {

    for (k in 1:n_distinct(x$C)) {
      
      count <- count + 1
      
      print(glue("Level {i} of A ({x$A[count]}), {j} of B ({x$B[count]}), {k} of C ({x$C[count]})"))
...
count <- 0
for (i in 1:n_distinct(x$C)) {

  for (j in 1:n_distinct(x$A)) {

    for (k in 1:n_distinct(x$B)) {
      
      count <- count + 1
      
      print(glue("Level {j} of A ({x$A[count]}), {k} of B ({x$B[count]}), {i} of C ({x$C[count]})"))
...
When models are run for each location and species included in the model, a bit difficult to interpret because of our migratory year classification. Only two months in 2019 (Apr, May) were affected by COVID, so is the big spike we see in 2019 just due to those months? Then does that mean 2020 was more normal despite the second wave? Or should we be considering the spike in 2019 as a pre-COVID spike and the pattern in 2020 as the COVID effect?

On including a third graph of all ten months other than the COVID months, it seems like this corroborates what we found from the initial metrics: that most changes were only in 2020, and despite the second wave in 2021 a lot of things were pretty much back to normal. This also answers my above question---we can safely consider the spike in MY 2019 as the effect of COVID. 

The months of April and May seem to generally (in MYs 2018 and 2021) be different from other months, with higher frequencies for both rural and urban species. This was more pronounced in the pandemic year, and negligible in MY 2020.

Spike in MY 2019: 
My first thought was much more birding + improved skill of birders (because there is a slight increase even in the non-lockdown months). But it's also coming back down...
I don't think it's more birds coming to human areas, because both urban and rural birds show similar increases. Then, could it really be better detection due to lower noise levels? Lower noise levels could have leaked into some of the non-lockdown months as well, explaining the presence of slight increase there.
