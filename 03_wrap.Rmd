...
temp <- data0_slice_G %>% 
  group_by(COVID, YEAR, MONTH, CELL.ID) %>% # state is meaningless cos CELL.ID
  filter(URBAN == 1) %>% 
...
##### Final analysis: GLMM of urban birding proportion ####

require(lme4)
load("data/data0_MY_slice.RData")

# taking no more than 100 lists for every space-time combo 
set.seed(420)
data_urban <- data0_MY_slice_G %>% 
  group_by(CELL.ID, M.YEAR, MONTH) %>% 
  slice_sample(n = 100) %>% 
  ungroup() %>% 
  select(URBAN, M.YEAR, MONTH, CELL.ID) %>% 
...
tictoc::tic("UNU GLMM fitting")
model_urban <- glmer(URBAN ~ M.YEAR + MONTH + MONTH:M.YEAR + (1|CELL.ID),
                     data = data_urban, family = binomial(link = "cloglog"),
                     nAGQ = 0, control = glmerControl(optimizer = "bobyqa")) # 244 sec
tictoc::toc()
...
urban0 <- data_urban %>% 
  tidyr::expand(MONTH, M.YEAR) %>% 
  mutate(PROP.U = NA)

count <- 0
for (j in 1:n_distinct(urban0$MONTH)) {
  for (k in 1:n_distinct(urban0$M.YEAR)) {
    count <- count + 1
    
    urban0$PROP.U[count] = predict(
      model_urban,
      data.frame(MONTH = urban0$MONTH[count],
                 M.YEAR = urban0$M.YEAR[count]),
      re.form = NA, 
      type = "response")
  }
}

##### trying with proportion ####

data_urban2 <- data0_MY_slice_G %>% 
...
# removing MONTH main effect unlike in birds model
tictoc::tic("UNU GLMM using proportions")
model_urban2 <- glmer(PROP.U ~ M.YEAR + M.YEAR:MONTH + (1|COUNTY/CELL.ID), 
                      weights = TOT.LISTS,
                      data = data_urban2, family = binomial(link = "cloglog"),
...
urban2 <- data_urban2 %>% 
  tidyr::expand(MONTH, M.YEAR) %>% 
  mutate(PROP.U.PRED = NA)

count <- 0
for (j in 1:n_distinct(urban2$MONTH)) {
  for (k in 1:n_distinct(urban2$M.YEAR)) {
    count <- count + 1
    
    urban2$PROP.U.PRED[count] = predict(
      model_urban2,
      data.frame(MONTH = urban2$MONTH[count],
                 M.YEAR = urban2$M.YEAR[count]),
      re.form = NA, 
      type = "response")
  }
}
...
# 
# ggplot(urban2, aes(M.YEAR, PROP.U.PRED, col = M.YEAR)) +
#     scale_color_manual(values = covid_palette, name = "Migratory year") +
...
## GLM(M)s for metrics of birder behaviour

```{r group_model1}

set.seed(420)
data_group1 <- data0_MY_slice_G %>% 
  group_by(M.YEAR, MONTH, OBSERVER.ID) %>% 
  filter(NUMBER.OBSERVERS > 1) %>% 
  dplyr::summarise(S.LISTS = n_distinct(GROUP.ID)) %>% 
  right_join(data0_MY_slice_G) %>% 
  group_by(M.YEAR, MONTH, OBSERVER.ID) %>% 
  dplyr::summarise(TOT.LISTS = n_distinct(GROUP.ID),
                   S.LISTS = replace_na(S.LISTS, 0),
                   PROP.U = round(S.LISTS/TOT.LISTS, 4)) %>% 
  # taking no more than 100 lists for every space-time combo 
  group_by(M.YEAR, MONTH, OBSERVER.ID) %>% 
  slice_sample(n = 100) %>% 
  # we need year and month as categorical variables
...
tictoc::tic("GLMM for UNU with weighted proportion response")
model_urban2 <- glmer(PROP.U ~ M.YEAR + MONTH + M.YEAR:MONTH + (1|CELL.ID),
                      weights = TOT.LISTS,
                      data = data_urban2, family = binomial(link = "cloglog"),
                      nAGQ = 0, control = glmerControl(optimizer = "bobyqa")) # 657 sec
tictoc::toc()
