...
data0_slice_G_MY <- data0_slice_G %>% 
  # adding UNU info to each checklist
  left_join((data_UNU %>% distinct(URBAN, CELL.ID, SUBCELL.ID, GROUP.ID)), 
            by = "GROUP.ID") %>% 
...
rm(data0_slice_G, data0_slice_S)


# bird species to analyse/compare
species <- data.frame(
  COMMON.NAME = c("House Crow", "Common Myna", "Jungle Myna", "Rock Pigeon", "Black Kite", "Common Tailorbird", "Rose-ringed Parakeet", "Asian Koel", "Purple Sunbird", "Pale-billed Flowerpecker", "White-cheeked Barbet", "Blue-throated Barbet",
  ...
  SP.CATEGORY = c(rep("U", 12), rep("W", 3), rep("R", 12)))

# Guwahati lacks PBFl, WCBa (but BTBa), InRo, GrFr.
...
birds_monthly <- data0_slice_G_MY %>% 
  ...
  group_by(COMMON.NAME, COVID, YEAR, MONTH) %>% 
...
plot_annotation(title = "Bengaluru Urban district") -> b_bang_01_monthly
...
Some strange patterns are present, though nothing conclusively due to the pandemic and not in one particular direction. Do these patterns persist at the annual level (considering SoIB scale of migratory year)?

```{r b_bang_02_annual, cache=TRUE, message=FALSE}

birds <- data0_slice_G_MY %>% 
  filter(COUNTY == "Bangalore", M.YEAR > 2018) %>% 
...
(ggplot(filter(birds, SP.CATEGORY == "W"), 
  ...
  labs(title = "Wetland species",
       x = "Migratory year", y = "Reporting frequency"))) +
plot_layout(guides = "collect", heights = c(4, 4, 1)) +
plot_annotation(title = "Bengaluru Urban district") -> b_bang_02_annual
...
birds_monthly <- data0_slice_G_MY %>% 
  ...
  left_join(data0) %>% 
  filter(COMMON.NAME %in% species$COMMON.NAME) %>% 
  group_by(COMMON.NAME, COVID, YEAR, MONTH, CELL.ID) %>% 
  summarise(TOT.LISTS = min(TOT.LISTS),
            NO.LISTS = n_distinct(GROUP.ID),
            REP.FREQ = round(NO.LISTS/TOT.LISTS, 4),
            # not bootstrapping here cos only averaging across 12 months
...
birds <- data0_slice_G_MY %>% 
  filter(STATE == "Karnataka", M.YEAR > 2018) %>% 
...
There is no clear direction of change in migratory year 2019. Need to check whether some of the patterns are "significant" (i.e., effect size).
...
(ggplot(filter(birds_monthly, SP.CATEGORY == "W"), 
  ...
  labs(title = "Wetland species",
       x = "Month", y = "Reporting frequency"))) +
plot_layout(guides = "collect", heights = c(4, 4, 1)) +
plot_annotation(title = "Pune district") -> b_pune_01_monthly
...
Non-urban bird species (8/12) showed a prominent decrease in reporting frequency during the "peak" period. 

```{r b_pune_02_annual, cache=TRUE, message=FALSE}

birds <- data0_slice_G_MY %>% 
  filter(COUNTY == "Pune", M.YEAR > 2018) %>% 
  group_by(M.YEAR, MONTH) %>% 
  summarise(GROUP.ID = GROUP.ID,
            SAMPLING.EVENT.IDENTIFIER = SAMPLING.EVENT.IDENTIFIER,
            TOT.LISTS = n_distinct(GROUP.ID)) %>% 
...
(ggplot(filter(birds, SP.CATEGORY == "W"), 
  ...
  labs(title = "Wetland species",
       x = "Migratory year", y = "Reporting frequency"))) +
plot_layout(guides = "collect", heights = c(4, 4, 1)) +
plot_annotation(title = "Pune district") -> b_pune_02_annual
...
They persist at the annual level for some species with high effect size. What happens when considering a larger spatial scale?
...
birds <- data0_slice_G_MY %>% 
  filter(STATE == "Maharashtra", M.YEAR > 2018) %>% 
  group_by(M.YEAR, MONTH, CELL.ID) %>% 
  summarise(GROUP.ID = GROUP.ID,
            SAMPLING.EVENT.IDENTIFIER = SAMPLING.EVENT.IDENTIFIER,
            TOT.LISTS = n_distinct(GROUP.ID)) %>% 
...
Except a couple of species that still show patterns of the pandemic impact, the majority was unaffected or showed the opposite patterns.

## Ernakulam

```{r b_eklm_01_monthly, cache=TRUE, message=FALSE}

# different from list for BLR and PUN because 5 non-urban species from that list do not occur in sufficient numbers in EKM
species_eklm <- data.frame(
  COMMON.NAME = c("House Crow", "Common Myna", "Jungle Myna", "Rock Pigeon", "Black Kite", "Common Tailorbird", "Rose-ringed Parakeet", "Asian Koel", "Purple Sunbird", "Pale-billed Flowerpecker", "White-cheeked Barbet", "Blue-throated Barbet",
                  "Little Cormorant", "Gray-headed Swamphen", "Indian Pond-Heron",
  ...
  SP.CATEGORY = c(rep("U", 12), rep("W", 3), rep("R", 10)))


birds_monthly <- data0_slice_G_MY %>% 
  filter(COUNTY == "Ernakulam") %>% 
  group_by(COVID, YEAR, MONTH) %>% 
  summarise(GROUP.ID = GROUP.ID, # not mutating so as to remove unwanted join columns
            SAMPLING.EVENT.IDENTIFIER = SAMPLING.EVENT.IDENTIFIER,
            TOT.LISTS = n_distinct(GROUP.ID)) %>% 
  left_join(data0) %>% 
  filter(COMMON.NAME %in% species_eklm$COMMON.NAME) %>%
  group_by(COMMON.NAME, COVID, YEAR, MONTH) %>% 
  summarise(TOT.LISTS = min(TOT.LISTS),
            NO.LISTS = n_distinct(GROUP.ID),
            REP.FREQ = round(NO.LISTS/TOT.LISTS, 4),
...
(ggplot(filter(birds_monthly, SP.CATEGORY == "W"), 
  ...
  labs(title = "Wetland species",
       x = "Month", y = "Reporting frequency"))) +
plot_layout(guides = "collect", heights = c(4, 4, 1)) +
plot_annotation(title = "Ernakulam district") -> b_eklm_01_monthly


ggsave("03_wrap_figs/b_eklm_01_monthly.png", b_eklm_01_monthly,
       width = 11, height = 15, units = "in", dpi = 300)

```

No prominent effect of the pandemic.

```{r b_eklm_02_annual, cache=TRUE, message=FALSE}

birds <- data0_slice_G_MY %>% 
  filter(COUNTY == "Ernakulam", M.YEAR > 2018) %>% 
  group_by(M.YEAR, MONTH) %>% 
  summarise(GROUP.ID = GROUP.ID,
            SAMPLING.EVENT.IDENTIFIER = SAMPLING.EVENT.IDENTIFIER,
            TOT.LISTS = n_distinct(GROUP.ID)) %>% 
...
(ggplot(filter(birds, SP.CATEGORY == "W"), 
  ...
  labs(title = "Wetland species",
       x = "Migratory year", y = "Reporting frequency"))) +
plot_layout(guides = "collect", heights = c(4, 4, 1)) +
plot_annotation(title = "Ernakulam district") -> b_eklm_02_annual


ggsave("03_wrap_figs/b_eklm_02_annual.png", b_eklm_02_annual,
       width = 9, height = 13, units = "in", dpi = 300)


```

Here, many strong patterns in urban species, showing lower reporting frequencies in migratory year 2021. In non-urban species a majority of the patterns are contradictory, with lower reporting frequencies in migratory year 2021. The same goes for wetland species.

This is a strange result, but maybe has to do with the ecology of these "urban" and "non-urban" species in Kerala. For example, treepies, drongos, kingfishers and bee eaters are a relatively common sight in cities. 

There is probably also an effect of the fact that EKM district is not dominated by urban areas, as BLR and PUN districts. Moreover, 2019 might have seen several birding events/surveys in Kerala.

```{r b_eklm_03_spatial, cache=TRUE, message=FALSE}
...
(ggplot(filter(birds_monthly, SP.CATEGORY == "W"), 
  ...
  labs(title = "Wetland species",
       x = "Month", y = "Reporting frequency"))) +
plot_layout(guides = "collect", heights = c(4, 4, 1)) +
plot_annotation(title = "Kerala state") -> b_eklm_03_spatial_m

ggsave("03_wrap_figs/b_eklm_03_spatial_m.png", b_eklm_03_spatial_m,
       width = 11, height = 15, units = "in", dpi = 300)



birds <- data0_slice_G_MY %>% 
  filter(STATE == "Kerala", M.YEAR > 2018) %>% 
  group_by(M.YEAR, MONTH, CELL.ID) %>% 
  summarise(GROUP.ID = GROUP.ID,
            SAMPLING.EVENT.IDENTIFIER = SAMPLING.EVENT.IDENTIFIER,
            TOT.LISTS = n_distinct(GROUP.ID)) %>% 
...
(ggplot(filter(birds, SP.CATEGORY == "W"), 
  ...
  labs(title = "Wetland species",
       x = "Migratory year", y = "Reporting frequency"))) +
plot_layout(guides = "collect", heights = c(4, 4, 1)) +
plot_annotation(title = "Kerala state") -> b_eklm_03_spatial_a


ggsave("03_wrap_figs/b_eklm_03_spatial_a.png", b_eklm_03_spatial_a,
       width = 10, height = 14, units = "in", dpi = 300)


```

Some urban species still show an effect of the pandemic, but the effect size would have to be judged. Non-urban and wetland species don't show any consistent directional change.
