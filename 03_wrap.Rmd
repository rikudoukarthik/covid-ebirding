...
data_b <- data0_MY_slice_G %>% 
  ...
  summarise(GROUP.ID = GROUP.ID,
            TOT.LISTS = n_distinct(GROUP.ID),
            AV.LENGTH = mean(NO.SP),
            MED.LENGTH = median(NO.SP)) %>% 
  left_join(data0_MY) %>% 
  filter(COMMON.NAME %in% species$COMMON.NAME) %>% 
  group_by(COMMON.NAME, M.YEAR, MONTH, CELL.ID) %>% 
  summarise(TOT.LISTS = min(TOT.LISTS),
            AV.LENGTH = min(AV.LENGTH),
            MED.LENGTH = min(MED.LENGTH),
            NO.LISTS = n_distinct(GROUP.ID),
            REP.FREQ = round(NO.LISTS/TOT.LISTS, 4)) %>% 
  left_join(species) %>% 
...
### to try directly with raw dat of presence-absence
...
temp2 <- data0_MY %>% 
  ...
  select(GROUP.ID, STATE, COUNTY, LOCALITY, LATITUDE, LONGITUDE, OBSERVATION.DATE, 
         YEAR, MONTH, DAY.M, M.YEAR, URBAN, CELL.ID, SUBCELL.ID, NO.SP)
...
bang1 <- glm(REPORT ~ M.YEAR + MONTH + SP.CATEGORY + NO.SP +
                 M.YEAR:MONTH + M.YEAR:SP.CATEGORY + MONTH:SP.CATEGORY,
               data = data_b2, family = binomial(link = 'cloglog'))
...
bang2 <- glmer(REPORT ~ M.YEAR + MONTH + SP.CATEGORY + NO.SP +
                 M.YEAR:MONTH + M.YEAR:SP.CATEGORY + MONTH:SP.CATEGORY +
                 (1|CELL.ID),
               data = data_b2, family = binomial(link = 'cloglog'),
...
bang3 <- glmer(REPORT ~ M.YEAR + MONTH + SP.CATEGORY + NO.SP +
                 M.YEAR:MONTH + M.YEAR:SP.CATEGORY +
                 (1|CELL.ID),
               data = data_b2, family = binomial(link = 'cloglog'),
               nAGQ = 0, control = glmerControl(optimizer = "bobyqa"))
