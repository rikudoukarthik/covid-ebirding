...
for (i in 1:n_distinct(prolific0$COMMON.NAME)) {
  ...
  data_spec <- data_prolific %>% 
    filter(COMMON.NAME == unique(prolific0$COMMON.NAME)[7]) %>% 
    # filtering for CELL.ID-MONTH (space-time) combos in which species occurs
    filter(REPORT == 1) %>% 
    distinct(COMMON.NAME, LOCALITY.ID, MONTH) %>% 
    left_join(data_prolific)
  ...
  for (j in 1:n_distinct(prolific0$MONTH)) {
    for (k in 1:n_distinct(prolific0$M.YEAR)) {
      ...
      print(glue::glue("Completed predictions for {unique(data_spec$COMMON.NAME)}, month {prolific0$MONTH[count]}, migratory year {prolific0$M.YEAR[count]}"))
    }
  }
}
