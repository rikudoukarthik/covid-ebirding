...
pred_a <- data_a %>% 
  ...
  mutate(PRED = predict(model_a, data.frame(MONTH, M.YEAR),
                        type = "response", re.form = NA)) %>% 
  left_join(timeline)

ggplot(pred_a, aes(MONTH, PRED, colour = COVID)) +
...
pred_b <- data_b %>% 
  tidyr::expand(nesting(MONTH, M.YEAR)) %>% 
  mutate(PRED = predict(model_b, data.frame(MONTH, M.YEAR),
                        type = "response", re.form = NA)) %>% 
  left_join(timeline)

ggplot(pred_b, aes(MONTH, PRED, colour = COVID)) +
...
pred_c <- data_c %>% 
  tidyr::expand(nesting(MONTH, M.YEAR)) %>% 
  mutate(PRED = predict(model_c, data.frame(MONTH, M.YEAR),
                        type = "response", re.form = NA)) %>% 
  left_join(timeline)

ggplot(pred_c, aes(MONTH, PRED, colour = COVID)) +
...
pred_d <- data_d %>% 
  tidyr::expand(nesting(MONTH, M.YEAR)) %>% 
  mutate(PRED = predict(model_d, data.frame(MONTH, M.YEAR),
                        type = "response", re.form = NA)) %>% 
  left_join(timeline)

ggplot(pred_d, aes(MONTH, PRED, colour = COVID)) +
...
data_a <- data0_MY_slice_S %>% 
  ...
  filter(MAIN.STATE == anal_states$a) %>% 
...
data_b <- data0_MY_slice_S %>% 
  left_join(obs_mainstates, by = "OBSERVER.ID") %>% 
  filter(MAIN.STATE == anal_states$b) %>% 
  group_by(M.YEAR, MONTH, OBSERVER.ID) %>% 
  dplyr::summarise(NO.SITES = n_distinct(CELL.ID)) %>% 
  mutate(NO.SITES = replace_na(NO.SITES, 0)) %>% 
  ungroup()
...
data_c <- data0_MY_slice_S %>% 
  left_join(obs_mainstates, by = "OBSERVER.ID") %>% 
  filter(MAIN.STATE == anal_states$c) %>% 
  group_by(M.YEAR, MONTH, OBSERVER.ID) %>% 
  dplyr::summarise(NO.SITES = n_distinct(CELL.ID)) %>% 
  mutate(NO.SITES = replace_na(NO.SITES, 0)) %>% 
  ungroup()
...
tictoc::tic("GLMM")
model_c <- glmer(NO.SITES ~ M.YEAR + M.YEAR:MONTH + (1|OBSERVER.ID),
                 data = data_c, family = poisson,
                 nAGQ = 0, control = glmerControl(optimizer = "bobyqa")) 
tictoc::toc()
...
data_d <- data0_MY_slice_S %>% 
  left_join(obs_mainstates, by = "OBSERVER.ID") %>% 
  filter(MAIN.STATE == anal_states$d) %>% 
  group_by(M.YEAR, MONTH, OBSERVER.ID) %>% 
  dplyr::summarise(NO.SITES = n_distinct(CELL.ID)) %>% 
  mutate(NO.SITES = replace_na(NO.SITES, 0)) %>% 
  ungroup()
...
tictoc::tic("GLMM")
model_d <- glmer(NO.SITES ~ M.YEAR + M.YEAR:MONTH + (1|OBSERVER.ID),
                 data = data_d, family = poisson,
                 nAGQ = 0, control = glmerControl(optimizer = "bobyqa")) 
tictoc::toc()
...
tictoc::tic("GLMM")
model_b <- glmer(B.TIME ~ M.YEAR + M.YEAR:MONTH + (1|OBSERVER.ID),
                 data = data_b, family = poisson,
                 nAGQ = 0, control = glmerControl(optimizer = "bobyqa")) 
tictoc::toc()
...
tictoc::tic("GLMM")
model_c <- glmer(B.TIME ~ M.YEAR + M.YEAR:MONTH + (1|OBSERVER.ID),
                 data = data_c, family = poisson,
                 nAGQ = 0, control = glmerControl(optimizer = "bobyqa")) 
tictoc::toc()
...
tictoc::tic("GLMM")
model_d <- glmer(B.TIME ~ M.YEAR + M.YEAR:MONTH + (1|OBSERVER.ID),
                 data = data_d, family = poisson,
                 nAGQ = 0, control = glmerControl(optimizer = "bobyqa")) 
tictoc::toc()
...
tictoc::tic("GLMM")
model_c <- glmer(HOTSPOT ~ M.YEAR + M.YEAR:MONTH + (1|CELL.ID),
                 data = data_c, family = binomial(link = "cloglog"),
                 nAGQ = 0, control = glmerControl(optimizer = "bobyqa")) 
tictoc::toc()
...
tictoc::tic("GLMM")
model_d <- glmer(HOTSPOT ~ M.YEAR + M.YEAR:MONTH + (1|CELL.ID),
                 data = data_d, family = binomial(link = "cloglog"),
                 nAGQ = 0, control = glmerControl(optimizer = "bobyqa")) 
tictoc::toc()
...
* Overall, this metric was not affected as much as others. Karnataka and Assam showed some brief declines, but overall patterns are rather irregular, so nothing major.
...
* Patterns somewhat similar to those of birding distance.
...
* Karnataka didn't show any change at all.  
