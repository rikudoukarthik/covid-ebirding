...
species_koch <- data.frame(
  COMMON.NAME = c("House Crow", "Common Myna", "Jungle Myna", "Rock Pigeon", "Black Kite", "Common Tailorbird", "Rose-ringed Parakeet", "Asian Koel", "Purple Sunbird", "Pale-billed Flowerpecker", "White-cheeked Barbet", 
                  "Little Cormorant", "Gray-headed Swamphen", "Indian Pond-Heron",
                  "Green Bee-eater", "Black Drongo", "Plain Prinia", "Indian Robin", "House Sparrow", "Coppersmith Barbet", "White-throated Kingfisher", "Black-rumped Flameback", "Rufous Treepie"),
  SP.CATEGORY = c(rep("U", 11), rep("W", 3), rep("R", 9)))
...
data_b2 <- data0_MY_slice_G %>% 
  ...
  # we need year and month as categorical variables
  ...
  ggplot(bang2, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
     ...
     geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                   size = 1.5, width = 0.2, position = position_dodge(0.5))) +
  plot_layout(guides = "collect") +
  plot_annotation(title = "Karnataka state",
                  subtitle = "Predicted reporting frequencies of 23 species") -> b_bang_04_model
...
for (i in 1:n_distinct(pune0$COMMON.NAME)) {
 ...
 x <- data_b2 %>% filter(COMMON.NAME == "Indian Robin")
 summary(as.factor(x$REPORT))
  ...
  ggplot(pune2, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
     ...
     geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                   size = 1.5, width = 0.2, position = position_dodge(0.5))) +
  plot_layout(guides = "collect") +
  plot_annotation(title = "Maharashtra state",
                  subtitle = "Predicted reporting frequencies of 23 species") -> b_pune_04_model
...
data_spec <- data_b2 %>% 
  filter(COMMON.NAME == unique(koch0$COMMON.NAME)[20])
...
for (j in 1:n_distinct(koch0$MONTH)) {
  for (k in 1:n_distinct(koch0$M.YEAR)) {
    ...
    koch00$REP.FREQ.PRED[count] = predict(
      model_spec,
      data.frame(COMMON.NAME = koch00$COMMON.NAME[count],
                 MONTH = koch00$MONTH[count],
                 M.YEAR = koch00$M.YEAR[count],
                 NO.SP = median_length$NO.SP.MED[koch00$MONTH[count]]),
      re.form = NA, 
      type = "response")
  }
}
 ...
 summary(as.factor(x$REPORT))
 summary(as.factor(data_spec$REPORT))
  ...
  ggplot(koch2, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
     ...
     geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                   size = 1.5, width = 0.2, position = position_dodge(0.5))) +
  plot_layout(guides = "collect") +
  plot_annotation(title = "Kerala state",
                  subtitle = "Predicted reporting frequencies of 23 species") -> b_koch_04_model
