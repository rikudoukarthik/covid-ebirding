...
temp <- data_prolific %>% 
  ...
  # filtering for species present in >= 10% of lists
  filter(PROP.LISTS >= 10) %>% 
  distinct(LOCALITY.ID, COMMON.NAME)

# # for removing species only present in one locality (because cannot use random effect in model)
# temp4 <- data_prolific %>% 
...
data_prolific <- data_prolific %>% 
  # filtering for patch-species combos present in >= 10% of lists
  right_join(temp) %>% 
...
count <- 0
for (l in 1:n_distinct(prolific00$LOCALITY.ID)) {
  
  prolific0 <- prolific00 %>%
    filter(LOCALITY.ID == unique(prolific00$LOCALITY.ID)[l])
  ...
  for (i in 1:n_distinct(prolific0$COMMON.NAME)) {
    ...
    model_spec <- glm(REPORT ~ M.YEAR + MONTH + MONTH:NO.SP + MONTH:M.YEAR,
                        data = data_spec, family = binomial(link = "cloglog"))
    ...
    for (j in 1:n_distinct(prolific0$MONTH)) {
      ...
      for (k in 1:n_distinct(prolific0$M.YEAR)) {
        ...
        prolific00$REP.FREQ.PRED[count] = predict(
          model_spec,
          data.frame(COMMON.NAME = prolific00$COMMON.NAME[count],
                     MONTH = prolific00$MONTH[count],
                     M.YEAR = prolific00$M.YEAR[count],
                     NO.SP = median_length$NO.SP.MED[prolific00$MONTH[count]]),
          re.form = NA, 
          type = "response")
        
        print(glue::glue("Predicted for loc. {l}, sp. {i} {unique(data_spec$COMMON.NAME)}, month {j}, migratory year {prolific0$M.YEAR[count]}"))
