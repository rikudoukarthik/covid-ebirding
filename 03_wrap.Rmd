...
data_b <- data0_MY_slice_G %>% 
  ...
  mutate(M.YEAR = as.factor(M.YEAR),
         MONTH = as.factor(MONTH)) %>% 
  ungroup()

# no building up of model from a "null" is required here as we are not trying to find which of our predictors is important; we want to know whether the predictors we think are important, actually are and whether they really had an impact on the data.

bang1 <- glm(REP.FREQ ~ M.YEAR + MONTH + SP.CATEGORY +
                 M.YEAR:MONTH + M.YEAR:SP.CATEGORY + MONTH:SP.CATEGORY,
               data = data_b, family = binomial(), weights = TOT.LISTS)

summary(bang1)
exp(coef(bang1) + coef(bang1)[1])[-1]
anova(bang1, test = "Chisq")
plot(bang1)


bang2 <- glmer(REP.FREQ ~ M.YEAR + MONTH + SP.CATEGORY +
                 M.YEAR:MONTH + M.YEAR:SP.CATEGORY + MONTH:SP.CATEGORY +
                 (1|CELL.ID),
               data = data_b, family = binomial(), weights = TOT.LISTS)
...
pune1 <- glm(REP.FREQ ~ M.YEAR + MONTH + SP.CATEGORY +
                 M.YEAR:MONTH + M.YEAR:SP.CATEGORY + MONTH:SP.CATEGORY,
               data = data_b, family = binomial(), weights = TOT.LISTS)

summary(pune1)
exp(coef(pune1) + coef(pune1)[1])[-1]
anova(bang1, test = "Chisq")
plot(bang1)

pune2 <- glm(REP.FREQ ~ M.YEAR + 
               # MONTH + SP.CATEGORY +
                 M.YEAR:MONTH + M.YEAR:SP.CATEGORY + MONTH:SP.CATEGORY,
               data = data_b, family = binomial(), weights = TOT.LISTS)
...
- From pune1, it seems that at the level of the migratory year the higher reporting frequency of urban species during the 3-4 crucial months is not visible. 
- Although RF of urban species increased (month:spcategory), that of non-urban species didn't change notably (month:MY). 
- With years, reporting frequency of urban species has simply been increasing steadily.
