...
data_prolific <- temp2 %>% 
  ...
  # for every list from a person's patch, adding all possible species
  left_join(temp3) %>% 
...
# calculating reporting frequency each patch-species combo to filter out very uncommon 
temp2a <- temp2 %>% 
...
# for removing species only "present" (threshold) in one locality 
# (because cannot use random effect in model)
temp4 <- data_prolific %>%
  right_join(temp) %>% 
  group_by(COMMON.NAME) %>%
  summarise(N.LOC = n_distinct(LOCALITY.ID)) %>%
  filter(N.LOC < 2)

data_prolific <- data_prolific %>% 
  # filtering for patch-species combos present in threshold lists
  right_join(temp) %>% 
  arrange(OBSERVER.ID, LOCALITY.ID, SAMPLING.EVENT.IDENTIFIER, COMMON.NAME) %>% 
  # removing species only present in one locality
  filter(!(COMMON.NAME %in% temp4$COMMON.NAME)) %>%
  # joining length, year and month metadata
...
median_length <- temp2 %>% 
  left_join(data0_MY_slice_S) %>% 
  distinct(LOCALITY.ID, M.YEAR, MONTH, GROUP.ID, NO.SP) %>% 
  group_by(LOCALITY.ID, MONTH) %>% 
  summarise(NO.SP.MED = median(NO.SP))


### run per species, location var in model ####

# dataframe with empty column to populate with looped values
# total rows: product of distinct values of predictors
prolific0 <- data_prolific %>% 
  tidyr::expand(COMMON.NAME, MONTH, M.YEAR) %>% 
  left_join(species_prolific) %>% 
  mutate(REP.FREQ.PRED = NA)

count <- 0
for (i in 1:n_distinct(prolific0$COMMON.NAME)) {
  
  data_spec <- data_prolific %>% 
    filter(COMMON.NAME == unique(prolific0$COMMON.NAME)[i])
  ...
  for (j in 1:n_distinct(prolific0$MONTH)) {
    
    for (k in 1:n_distinct(prolific0$M.YEAR)) {
      
      count <- count + 1
      
      prolific0$REP.FREQ.PRED[count] = predict(
        model_spec,
        data.frame(COMMON.NAME = prolific0$COMMON.NAME[count],
      ...
      print(glue::glue("Completed predictions for sp. {i} {unique(data_spec$COMMON.NAME)}, month {prolific0$MONTH[count]}, migratory year {prolific0$M.YEAR[count]}"))
      
    }
  }
}

# at 20% threshold CoBa model singular fit for random effect (zero random variance).
# present in 2 locations, as 3 other species, but CoBa only one with singular fit.
...
(ggplot(prolific1, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    labs(title = "For the months of April and May",
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                  size = 1.5, width = 0.2, position = position_dodge(0.5)) |
  ggplot(prolific2, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
     labs(title = "For all twelve months",
  ...
  plot_annotation(title = "Bird species reporting in the pandemic, with constant effort",
                  subtitle = paste0(
                    "Predicted reporting frequencies of ",
                    n_distinct(prolific0$COMMON.NAME), " species (",
                    n_distinct(filter(prolific0,SP.CATEGORY == "U")$COMMON.NAME), " urban, ",
                    n_distinct(filter(prolific0,SP.CATEGORY == "R")$COMMON.NAME), " rural)")
                  ) -> b_prolific
...
### run per location, spec var in model ####

# filtering out location without both species categories
temp5 <- data_prolific %>% 
  left_join(species_prolific) %>% 
  group_by(LOCALITY.ID) %>% 
  summarise(N.SPC = n_distinct(SP.CATEGORY)) %>% 
  filter(N.SPC == 1) %>% 
  ungroup()

data_prolific <- data_prolific %>% 
  left_join(species_prolific) %>% 
  filter(!(LOCALITY.ID %in% temp5$LOCALITY.ID))

data_prolific1 <- data_prolific %>% filter(MONTH %in% 4:5) 
data_prolific2 <- data_prolific %>% filter(!(MONTH %in% 4:5))
data_prolific3 <- data_prolific
...
prolific0 <- data_prolific %>% 
  left_join(species_prolific) %>% 
  group_by(LOCALITY.ID) %>% 
  tidyr::expand(SP.CATEGORY, MONTH, M.YEAR) %>% 
  ungroup() %>% 
  mutate(REP.FREQ.PRED = NA)
  ...
  for (i in 1:n_distinct(prolific0$MONTH)) {
    ...
    for (j in 1:n_distinct(prolific0$M.YEAR)) {
      ...
      for (k in 1:n_distinct(prolific0$SP.CATEGORY)) {
        ...
        print(glue::glue("Completed predictions for loc. {l}, month {prolific0$MONTH[count]}, migratory year {prolific0$M.YEAR[count]}"))
...
(ggplot(prolific01, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                  size = 1.5, width = 0.2, position = position_dodge(0.5)) |
  ggplot(prolific02, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    scale_y_continuous(limits = c(0.1, 0.7)) +
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                   size = 1.5, width = 0.2, position = position_dodge(0.5)) |
  ggplot(prolific03, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    scale_y_continuous(limits = c(0.1, 0.7)) +
  ...
  plot_layout(guides = "collect") +
  plot_annotation(title = "Bird species reporting in the pandemic, with constant effort",
                  subtitle = "Predicted reporting frequencies of common urban and rural species, from locations with consistent effort. (One model, visual comparison of months.)") -> b_prolific_loc0

ggsave("03_wrap_figs/b_prolific_loc0.png", b_prolific_loc0,
...
prolific1 <- data_prolific1 %>% 
  left_join(species_prolific) %>% 
  group_by(LOCALITY.ID) %>% 
  tidyr::expand(nesting(MONTH), SP.CATEGORY, M.YEAR) %>% 
  ungroup() %>% 
  mutate(REP.FREQ.PRED = NA)

prolific2 <- data_prolific2 %>% 
  left_join(species_prolific) %>% 
  group_by(LOCALITY.ID) %>% 
  tidyr::expand(nesting(MONTH), SP.CATEGORY, M.YEAR) %>% 
  ungroup() %>% 
      ...
      for (k in 1:n_distinct(prolific1$SP.CATEGORY)) {
        ...
        prolific1$REP.FREQ.PRED[count] = predict(
          ...
          data.frame(LOCALITY.ID = prolific1$LOCALITY.ID[count],
                     ...
                     NO.SP = median_length$NO.SP.MED[prolific1$MONTH[count]],
...
count <- 0
for (l in 1:n_distinct(prolific2$LOCALITY.ID)) {
  
  data_loc <- data_prolific2 %>% 
    left_join(species_prolific) %>% 
    filter(LOCALITY.ID == unique(prolific2$LOCALITY.ID)[l])
  ...
  for (i in 1:n_distinct(prolific2$MONTH)) {
    
    for (j in 1:n_distinct(prolific2$M.YEAR)) {
      
      for (k in 1:n_distinct(prolific2$SP.CATEGORY)) {
        
        count <- count + 1
        
        prolific2$REP.FREQ.PRED[count] = predict(
          model_spec,
          data.frame(LOCALITY.ID = prolific2$LOCALITY.ID[count],
                     MONTH = prolific2$MONTH[count],
                     M.YEAR = prolific2$M.YEAR[count],
                     NO.SP = median_length$NO.SP.MED[prolific2$MONTH[count]],
                     SP.CATEGORY = prolific2$SP.CATEGORY[count]),
          re.form = NA, 
          type = "response")
        
        print(glue::glue("Completed predictions for loc. {l}, month {prolific2$MONTH[count]}, migratory year {prolific2$M.YEAR[count]}"))
...
# all months
prolific33 <- prolific03


(ggplot(prolific11, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    scale_y_continuous(limits = c(0.1, 0.7)) +
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                  size = 1.5, width = 0.2, position = position_dodge(0.5)) |
  ggplot(prolific22, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    scale_y_continuous(limits = c(0.1, 0.7)) +
    ...
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                   size = 1.5, width = 0.2, position = position_dodge(0.5)) |
  ggplot(prolific33, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    scale_color_manual(values = c("#8F85C1", "#A3383C"),
                       name = "Species category",
                       labels = c("Rural", "Urban")) +
    scale_y_continuous(limits = c(0.1, 0.7)) +
  ...
  plot_layout(guides = "collect") +
  plot_annotation(title = "Bird species reporting in the pandemic, with constant effort",
                  subtitle = "Predicted reporting frequencies of common urban and rural species, from locations with consistent effort. (Individual models for comparison of months.)") -> b_prolific_loc1

ggsave("03_wrap_figs/b_prolific_loc1.png", b_prolific_loc1,
       width = 21, height = 6, units = "in", dpi = 300)
