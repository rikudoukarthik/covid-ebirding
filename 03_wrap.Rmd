...
rm(areag1, areag2, areag3, areag4, districtmap)
...
3.  Spatial spread/evenness (24kmx24km cells; across 7 months) Large spatial and temporal scale---no STATE or MONTH/M.YEAR in question.
...
data_SoIBmap0 <- gridmapg1_sf %>% 
  left_join(data0_MY_slice_G) %>% 
...
transition_names <- data.frame(T.CODE = c("T1", "T2", "T3"),
                               T.LABEL = c("Before to During", "During to After", 
                                         "Before to After"))

data_SoIBmap <- data_SoIBmap1 %>% 
  bind_rows(data_SoIBmap2) %>% 
  bind_rows(data_SoIBmap3) %>% 
  pivot_wider(names_from = "COVID", values_from = "NO.LISTS") %>% 
  mutate(T1 = rast_logpropchange(BEF, DUR, emptycheck = T),
         T2 = rast_logpropchange(DUR, AFT, emptycheck = T),
         T3 = rast_logpropchange(BEF, AFT, emptycheck = T)) %>% 
  pivot_longer(cols = c(T1, T2, T3), names_to = "T.CODE", values_to = "T.CHANGE") %>% 
  left_join(transition_names) %>% 
  mutate(T.LABEL = factor(T.LABEL, levels = c("Before to During", "During to After", 
                                              "Before to After")))


s_spread_map <- ggplot(data_SoIBmap) +
  geom_polygon(data = indiamap,
               aes(long, lat, group = group),
               colour = "black", fill = "#E6E9F1", size = 0.2) +
  geom_sf(aes(fill = T.CHANGE), col = NA, na.rm = T) +
  facet_wrap(~ T.LABEL) +
  scale_fill_viridis_c(option = "cividis",
                       labels = ~ glue::glue("{10^(.x)}\u00d7"),
                       name = "Proportional\nchange",
                       # # failed attempt at rescaling to have mid-colour at midpoint
                       # rescaler = ~ scales::rescale(c(min(.x, na.rm = T), 
                       #                                exp(1), 
                       #                                max(.x, na.rm = T)))
                       ) +
  labs(x = "Longitude", y = "Latitude",
       title = "Proportional change in monthly no. of lists per 25km \u00d7 25km cell") +
  # making the colourbar bigger
  theme(legend.key.height = unit(1, "inches")) # height between two breaks of legend

ggsave("03_wrap_figs/10c_s_spread_map.png", s_spread_map,
...
# not directly using data_SoIBmap object because backtransforming after bootstrapping mean 
# and SE is not wise
net_effort <- data_SoIBmap1 %>% 
  bind_rows(data_SoIBmap2) %>% 
  bind_rows(data_SoIBmap3) %>% 
  pivot_wider(names_from = "COVID", values_from = "NO.LISTS") %>% 
  mutate(T1 = rast_propchange(BEF, DUR, emptycheck = T),
         T2 = rast_propchange(DUR, AFT, emptycheck = T),
         T3 = rast_propchange(BEF, AFT, emptycheck = T)) %>% 
  pivot_longer(cols = c(T1, T2, T3), names_to = "T.CODE", values_to = "T.CHANGE") %>% 
  left_join(transition_names) %>% 
  mutate(T.LABEL = factor(T.LABEL, levels = c("Before to During", "During to After", 
                                              "Before to After"))) %>% 
  ...
  dplyr::summarise(T.CHANGE = boot_conf(x = T.CHANGE)) %>% 
  group_by(T.CODE, T.LABEL) %>% 
  dplyr::summarise(CI.L = stats::quantile(T.CHANGE, 0.025), # Obtain the CIs
                   CI.U = stats::quantile(T.CHANGE, 0.975),
                   T.CHANGE = mean(T.CHANGE)) 

s_spread_netchange <- ggplot(net_effort, aes(x = T.LABEL, y = T.CHANGE)) + 
  geom_point(size = 4, position = position_dodge(0.5)) +
  geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                size = 1.75, width = 0.08, position = position_dodge(0.5)) +
  # scale_y_continuous(limits = c(1.1, 2.3), breaks = seq(1, 3, 0.2)) +
  labs(title = "Net proportional change in monthly birding effort (no. of lists)",
       subtitle = "Change per 25km \u00d7 25km cell averaged across all cells with birding activity",
       x = "Transition", y = "Proportional change") 

ggsave("03_wrap_figs/10c_s_spread_netchange.png", 
       plot = s_spread_netchange,
       dpi = 300, width = 6, height = 9, units = "in")


##### Final analysis: spatial autocorrelation with Moran's I ####
...
# for plotting raw data with alpha
prolific1a_points <- prolific1 %>% 
  filter(MONTH %in% 4:5) %>% 
  group_by(M.YEAR, SP.CATEGORY, LOCALITY.ID) %>% 
  summarise(REP.FREQ.PRED = boot_conf(REP.FREQ.PRED)) %>% 
  group_by(M.YEAR, SP.CATEGORY, LOCALITY.ID) %>% 
  summarise(SE = sd(REP.FREQ.PRED),
            REP.FREQ.PRED = mean(REP.FREQ.PRED)) %>% 
  # to make lines connect per location
  group_by(LOCALITY.ID, SP.CATEGORY) %>% 
  arrange(LOCALITY.ID, SP.CATEGORY, M.YEAR) %>% 
  mutate(PATH.GROUP = glue("{LOCALITY.ID}{SP.CATEGORY}"))

# main points to be plotted
prolific1a <- prolific1a_points %>% 
  group_by(M.YEAR, SP.CATEGORY) %>% 
...
(ggplot(prolific1a, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    ...
    labs(title = "For the months of April and May",
         x = "Migratory year", y = "Predicted reporting frequency") +
    # data points
    geom_point(data = prolific1a_points, aes(group = PATH.GROUP),
               size = 3, alpha = 0.25,
               position = position_dodge(0.25)) + 
    geom_path(data = prolific1a_points, 
  ...
  ggplot(prolific1b, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    ...
    labs(title = "For other ten months",
         x = "Migratory year", y = "Predicted reporting frequency") +
    # data points
    geom_point(data = prolific1b_points, aes(group = PATH.GROUP),
               size = 3, alpha = 0.25,
               position = position_dodge(0.25)) + 
    geom_path(data = prolific1b_points, 
              aes(x = as.numeric(M.YEAR), y = REP.FREQ.PRED, group = PATH.GROUP),
              size = 1, alpha = 0.15, position = position_dodge(0.25)) + 
    # main points
    geom_point(size = 3, position = position_dodge(0.5)) +
  ...
  ggplot(prolific1c, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    ...
    labs(title = "For all twelve months",
         x = "Migratory year", y = "Predicted reporting frequency") +
    # data points
    geom_point(data = prolific1c_points, aes(group = PATH.GROUP),
               size = 3, alpha = 0.25,
               position = position_dodge(0.25)) + 
    geom_path(data = prolific1c_points, 
              aes(x = as.numeric(M.YEAR), y = REP.FREQ.PRED, group = PATH.GROUP),
              size = 1, alpha = 0.15, position = position_dodge(0.25)) + 
    # main points
    geom_point(size = 3, position = position_dodge(0.5)) +
    geom_errorbar(aes(ymin = CI.L, ymax = CI.U), 
                  size = 1.5, width = 0.2, position = position_dodge(0.5))) +
  plot_layout(guides = "collect") +
  plot_annotation(title = "Change in bird species reporting from one model of all months",
                  subtitle = "Predicted reporting frequencies of common urban and rural species, from locations with consistent effort.") -> b_prolific1
...
prolific2c_points <- prolific1c_points
...
(ggplot(prolific2a, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    ...
    labs(title = "For the months of April and May",
         x = "Migratory year", y = "Predicted reporting frequency") +
    # data points
    geom_point(data = prolific2a_points, aes(group = PATH.GROUP),
               size = 3, alpha = 0.25,
               position = position_dodge(0.25)) + 
    geom_path(data = prolific2a_points, 
              aes(x = as.numeric(M.YEAR), y = REP.FREQ.PRED, group = PATH.GROUP),
              size = 1, alpha = 0.15, position = position_dodge(0.25)) + 
    # main points
    geom_point(size = 3, position = position_dodge(0.5)) +
  ...
  ggplot(prolific2b, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    ...
    labs(title = "For other ten months",
         x = "Migratory year", y = "Predicted reporting frequency") +
    # data points
    geom_point(data = prolific2b_points, aes(group = PATH.GROUP),
               size = 3, alpha = 0.25,
               position = position_dodge(0.25)) + 
    geom_path(data = prolific2b_points, 
              aes(x = as.numeric(M.YEAR), y = REP.FREQ.PRED, group = PATH.GROUP),
              size = 1, alpha = 0.15, position = position_dodge(0.25)) + 
    # main points
    geom_point(size = 3, position = position_dodge(0.5)) +
  ...
  ggplot(prolific2c, aes(M.YEAR, REP.FREQ.PRED, col = SP.CATEGORY)) +
    ...
    labs(title = "For all twelve months",
         x = "Migratory year", y = "Predicted reporting frequency") +
    # data points
    geom_point(data = prolific2c_points, aes(group = PATH.GROUP),
               size = 3, alpha = 0.25,
               position = position_dodge(0.25)) + 
    geom_path(data = prolific2c_points, 
              aes(x = as.numeric(M.YEAR), y = REP.FREQ.PRED, group = PATH.GROUP),
              size = 1, alpha = 0.15, position = position_dodge(0.25)) + 
    # main points
    geom_point(size = 3, position = position_dodge(0.5)) +
  ...
  plot_annotation(title = "Change in bird species reporting from three separate monthwise models",
                  subtitle = "Predicted reporting frequencies of common urban and rural species, from locations with consistent effort.") -> b_prolific2
